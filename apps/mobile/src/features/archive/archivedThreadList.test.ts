import type { ArchivedSnapshotEntry } from "@pkfactory/client-runtime/state/threads";
import type { OrchestrationProjectShell, OrchestrationThreadShell } from "@pkfactory/contracts";
...
describe("buildArchivedThreadGroups", () => {
  it("groups archived threads by project and sorts newest first", () => {
    const project = makeProject({ id: ProjectId.make("project-1"), title: "PK Factory" });
  ...
  it("filters by environment and matches project, thread, and branch text", () => {
    ...
    const firstProject = makeProject({ id: ProjectId.make("project-1"), title: "PK Factory" });
