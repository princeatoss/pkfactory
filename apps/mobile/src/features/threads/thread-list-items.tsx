...
import type { EnvironmentThreadSearchMatch } from "@pkfactory/client-runtime/state/thread-search";
import type { EnvironmentMachineKind } from "@pkfactory/contracts";
    ...
    >
      ...
      >
        ...
        <Text
          className={
            compact
              ? "flex-shrink text-base font-pkfactory-bold tracking-[0.2px] text-foreground-muted"
              : "flex-shrink text-sm font-pkfactory-bold tracking-[0.2px] text-foreground-muted"
          }
          numberOfLines={1}
        >
          {props.title}
        ...
        <Text
          className={
            compact
              ? "flex-1 text-sm font-pkfactory-medium text-foreground-tertiary"
              : "flex-1 text-xs font-pkfactory-medium text-foreground-tertiary"
          }
        >
          {props.threadCount}
        </Text>
    ...
    >
      <View className="flex-row items-center gap-1.5">
        ...
        <Text
          className={
            compact
              ? "text-sm font-pkfactory-medium text-foreground-muted"
              : "text-xs font-pkfactory-medium text-foreground-muted"
          }
        >
          {label}
        </Text>
...
export const PendingTaskListRow = memo(function PendingTaskListRow(props: {
  ...
  const statusPill = (
    <View className="rounded-full bg-adaptive-zinc-500-a12-a16 px-1.5 py-0.5">
      <Text className="text-3xs font-pkfactory-bold text-adaptive-zinc-600-300">Pending</Text>
    </View>
  );
  ...
  const rowContent = compact ? (
      ...
      <View className="pr-[18px] pt-[10px]" style={{ paddingLeft: THREAD_LIST_COMPACT_INSET }}>
        <View className={cn("gap-[3px] pb-[10px]", !props.isLast && "border-b border-separator")}>
          <View className="flex-row items-center justify-between gap-2">
            <Text className="flex-1 text-lg font-pkfactory-bold text-foreground" numberOfLines={1}>
              {pendingTask.title}
            </Text>
    ...
    >
      <View className="gap-[3px]">
        <View className="flex-row items-center justify-between gap-2">
          <Text
            className="flex-1 text-base font-pkfactory-medium text-foreground"
            numberOfLines={1}
          >
            {pendingTask.title}
          </Text>
...
export const ThreadListRow = memo(function ThreadListRow(props: {
  ...
  const statusPill = effectiveStatus ? (
    <View className={`${effectiveStatus.pillClassName} rounded-full px-1.5 py-0.5`}>
      <Text className={`text-3xs font-pkfactory-bold ${effectiveStatus.textClassName}`}>
      ...
      <View className="mt-px flex-row items-center gap-1.5">
        ...
        {pr !== null ? (
          <View className="flex-row items-center gap-0.5">
            ...
            />
            <Text
              className={`${compact ? "text-sm" : "text-xs"} font-pkfactory-medium ${
      ...
      >
        <View className="pr-[18px] pt-[10px]" style={{ paddingLeft: THREAD_LIST_COMPACT_INSET }}>
          <View className={cn("gap-[3px] pb-[10px]", !props.isLast && "border-b border-separator")}>
            <View className="flex-row items-center justify-between gap-2">
              <Text
                className="flex-1 text-lg font-pkfactory-bold text-foreground"
                numberOfLines={1}
              >
                {thread.title}
              </Text>
      ...
      >
        <View className="gap-[3px]">
          <View className="flex-row items-center justify-between gap-2">
            <Text
              className={cn(
                "flex-1 text-base font-pkfactory-medium",
