...
export function SheetActionButton(props: {
  ...
  return (
    ...
    >
      ...
      <Text
        className={cn(
          "text-xs font-pkfactory-bold tracking-[0.9px] uppercase",
          tone === "primary"
...
export function MetaCard(props: { readonly label: string; readonly value: string }) {
  return (
    <View className="rounded-[18px] border border-border bg-card px-4 py-3">
      <Text className="text-foreground-muted text-2xs font-pkfactory-bold tracking-[0.9px] uppercase">
        {props.label}
      </Text>
  ...
  return (
    ...
    >
      ...
      <View className="flex-1 gap-0.5">
        <Text className="text-foreground text-base font-pkfactory-bold">{props.title}</Text>
        {props.subtitle ? (
