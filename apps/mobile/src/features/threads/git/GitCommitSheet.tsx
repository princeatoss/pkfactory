      ...
      >
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            ...
            <Text className="text-foreground text-base font-pkfactory-bold">
        ...
        <View className="gap-3 rounded-[22px] border border-border bg-card px-4 py-4">
          <View className="flex-row items-center justify-between gap-3">
            <View className="gap-1">
              <Text className="text-foreground text-base font-pkfactory-bold">Files</Text>
            ...
            <View className="flex-row items-center gap-2">
              {!allSelected && isEditingFiles ? (
                ...
                >
                  <Text className="text-foreground text-2xs font-pkfactory-bold uppercase">
                    Reset
                  </Text>
                </Pressable>
              ) : null}
            ...
            <View className="gap-2">
              {selectedFilePreview.map((file) => (
                <View key={file.path} className="flex-row items-center justify-between gap-3">
                  ...
                  <Text className="text-xs font-pkfactory-bold text-emerald-500">
                    +{file.insertions}
                  </Text>
                  <Text className="text-xs font-pkfactory-bold text-rose-500">
                    -{file.deletions}
                  </Text>
                </View>
              ))}
                  ...
                  >
                    ...
                    <View className="flex-row items-start justify-between gap-3">
                      <View className="flex-1 gap-1">
                        <Text
                          selectable
                          className={`text-sm font-pkfactory-bold ${included ? "text-foreground" : "text-foreground-muted"}`}
                        >
                          {file.path}
                        </Text>
                        {!included ? (
        ...
        <View className="gap-2">
          <Text className="text-foreground text-sm font-pkfactory-bold">Commit message</Text>
          <TextInput
            multiline
