...
import { BUILT_IN_THEMES, getThemeColorsForAppearance } from "@pkfactory/shared/themePalettes";
...
describe("getMobileTerminalTheme", () => {
  it("preserves the Pierre terminal for the default theme", () => {
    for (const scheme of ["light", "dark"] as const) {
      expect(getMobileTerminalTheme("pkfactory", scheme)).toEqual(getPierreTerminalTheme(scheme));
    }
  });

  it("applies the selected palette without replacing ANSI status colors", () => {
    const standard = getMobileTerminalTheme("pkfactory", "dark");
