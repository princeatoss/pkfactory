...
import {
  ...
  isWindowsPlatform,
} from "@pkfactory/client-runtime/state/projects";
import {
  CommandId,
  type EnvironmentId,
...
function SectionTitle(props: { readonly children: string }) {
  return (
    <Text className="px-1 text-2xs font-pkfactory-bold tracking-[0.7px] uppercase text-foreground-muted">
      {props.children}
    </Text>
  );
}
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        <View className="flex-1 gap-0.5">
          <Text className="text-base leading-snug font-pkfactory-bold">{props.title}</Text>
          {props.subtitle ? (
  ...
  return (
    ...
    >
      {props.loading ? (
        ...
        <Text className="text-base font-pkfactory-bold text-primary-foreground">{props.label}</Text>
      )}
    </Pressable>
  );
}
...
function EmptyEnvironmentState() {
  ...
  return (
    <View className="items-center gap-3 rounded-2xl bg-card px-5 py-8">
      <Text className="text-center text-lg font-pkfactory-bold">Environment unavailable</Text>
      ...
      >
        <Text className="text-sm font-pkfactory-bold text-primary-foreground">Add environment</Text>
      </Pressable>
    </View>
  );
}
    ...
    <AddProjectShell>
      ...
      {repositoryTitle ? (
        <View className="rounded-[24px] bg-card px-4 py-3">
          <Text className="text-base font-pkfactory-bold">{repositoryTitle}</Text>
