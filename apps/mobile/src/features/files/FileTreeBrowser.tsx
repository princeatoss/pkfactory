import type { ProjectEntry } from "@pkfactory/contracts";
    ...
    >
      ...
      <Text
        className={cn(
          ...
          props.selected
            ? "font-pkfactory-bold text-foreground"
            : "font-pkfactory-medium text-foreground-secondary",
        )}
        numberOfLines={1}
      >
        {node.name}
      ...
      {node.kind === "directory" ? (
        <Text className="text-2xs font-pkfactory-medium text-foreground-tertiary">
  ...
  if (props.error && props.entries.length === 0) {
    return (
      <View className="flex-1 bg-sheet px-4 py-5">
        <Text className="text-sm font-pkfactory-bold text-foreground">Files unavailable</Text>
      ...
      ListEmptyComponent={
        <View className="px-4 py-5">
          {props.isPending ? (
            ...
            <>
              <Text className="text-sm font-pkfactory-bold text-foreground">No files found</Text>
