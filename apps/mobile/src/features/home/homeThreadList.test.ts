...
import { EnvironmentId, ProjectId, ProviderInstanceId, ThreadId } from "@pkfactory/contracts";
...
describe("buildHomeThreadGroups", () => {
  it("builds one v2 scope for the same repository across environments", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        ...
        remoteName: "origin",
        remoteUrl: "git@github.com:princeatoss/pkfactory.git",
      },
    };
    const projects = [
      makeProject({
    ...
    expect(scopes[0]?.title).toBe("pkfactory");
  ...
  it("routes stale duplicate project refs through the canonical repository group", () => {
    ...
    const local = makeProject({
      ...
      environmentId: localEnvironmentId,
      title: "pkfactory",
      workspaceRoot: "/workspaces/pkfactory",
      repositoryIdentity,
    });
    const stale = makeProject({
      ...
      title: "pkfactory",
      workspaceRoot: "/remote/pkfactory",
      updatedAt: "2026-06-01T00:00:00.000Z",
    });
  ...
  it("matches web repository, repository-path, and separate grouping modes", () => {
    ...
    const repositoryIdentity = {
      ...
      locator: {
        source: "git-remote" as const,
        remoteName: "origin",
        remoteUrl: "git@github.com:pkfactorytools/pkfactory.git",
      },
      provider: "github",
      owner: "pkfactorytools",
      name: "pkfactory",
      displayName: "PK Factory",
      rootPath: "/workspaces/pkfactory",
    };
    const projects = [
      makeProject({
        environmentId,
        ...
        title: "Web",
        workspaceRoot: "/workspaces/pkfactory/apps/web",
        repositoryIdentity,
      }),
      makeProject({
        environmentId,
        ...
        title: "Mobile",
        workspaceRoot: "/workspaces/pkfactory/apps/mobile",
        repositoryIdentity,
      }),
    ];
    const threads = projects.map((project) =>
  ...
  it("does not apply the recency window while searching", () => {
    ...
    const group = buildGroups([project], threads, { searchQuery: "PK Factory" })[0];
