...
import { BUILT_IN_THEME_IDS, BUILT_IN_THEMES } from "@pkfactory/shared/themePalettes";
...
describe("mobile themes", () => {
  ...
  it("changes either theme without switching the active appearance", () => {
    const themeIds = { light: "pkfactory-chat", dark: "grove" } as const;
    expect(createMobileThemeSelectionPatch(themeIds, "light", "dark", "ocean")).toEqual({
      lightThemeId: "pkfactory-chat",
      darkThemeId: "ocean",
      themeId: "pkfactory-chat",
    });
