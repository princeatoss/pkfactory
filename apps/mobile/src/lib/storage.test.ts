...
describe("mobile connection storage", () => {
  ...
  it("preserves secure-storage read failures with operation and key context", async () => {
    ...
    await expect(loadSavedConnections()).rejects.toMatchObject({
      ...
      operation: "read",
      key: "pkfactory.connections",
      cause,
      message: "Mobile secure storage operation read failed for key pkfactory.connections.",
    });
  });

  it("logs structured decode failures before using the empty fallback", async () => {
    await mocks.setItemAsync("pkfactory.connections", "{");
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: "Failed to decode mobile storage value for key pkfactory.connections.",
      }),
    );
  ...
  it("loads legacy preferences when SQLite is unavailable", async () => {
    ...
    await mocks.setItemAsync("pkfactory.preferences", JSON.stringify({ baseFontSize: 17 }));
  ...
  it("falls back to secure storage when SQLite cannot save preferences", async () => {
    ...
    const fallback = JSON.parse(mocks.getStoredValue("pkfactory.preferences.fallback") ?? "") as {
  ...
  it("reconciles fallback preferences after SQLite recovers", async () => {
    ...
    await mocks.setItemAsync(
      "pkfactory.preferences.fallback",
      JSON.stringify({
    ...
    expect(mocks.getStoredValue("pkfactory.preferences.fallback")).toBeNull();
  });
  ...
  it("keeps SQLite authoritative when stale legacy preferences remain", async () => {
    mocks.setPreferencesJson(JSON.stringify({ baseFontSize: 21 }), 30);
    await mocks.setItemAsync("pkfactory.preferences", JSON.stringify({ baseFontSize: 19 }));
