# PK Factory Mobile Terminal Native Module
...
custom-I/O and live-padding branch. `PKFactoryTerminalView` owns a `libghostty` surface and uses that
callback I/O model:
...
`pkfactory/custom-io-ordered-feed` branch when vendored, based on VVTerm's
`vvterm/custom-io-padding` branch).

```bash
apps/mobile/modules/pkfactory-terminal/scripts/build-libghostty-ios16.sh
```
...
```bash
apps/mobile/modules/pkfactory-terminal/scripts/build-libghostty-android.sh
```
