...
public final class PKFactoryKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("PKFactoryKeyboardCommands")

    View(PKFactoryKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: PKFactoryKeyboardCommandsView, commands: [String]) in
...
public final class PKFactoryKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.pkfactoryFirstResponder == nil else { return }
...
private extension UIView {
  var pkfactoryFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.pkfactoryFirstResponder { return responder }
    }
    return nil
  }
}
