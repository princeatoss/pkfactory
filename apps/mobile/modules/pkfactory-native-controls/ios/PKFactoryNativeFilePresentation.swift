...
final class PKFactoryNativeFilePresentation: NSObject, QLPreviewControllerDataSource,
  QLPreviewControllerDelegate, UIAdaptivePresentationControllerDelegate {
  let identifier: String
  private var controller: UIViewController?
  private let completion: (Error?) -> Void
  private weak var sources: PKFactoryPresentationSources?
  ...
  init(identifier: String, sources: PKFactoryPresentationSources, sourceIdentifier: String, completion: @escaping (Error?) -> Void) {
  ...
  nonisolated private static func prepareFile(url: URL, title: String) async throws -> URL {
    ...
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("pkfactory-preview-\(UUID().uuidString)")
