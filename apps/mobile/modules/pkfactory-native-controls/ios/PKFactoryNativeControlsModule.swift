...
public final class PKFactoryNativeControlsModule: Module {
  private let presentationSources = PKFactoryPresentationSources()
  private var videoPresentation: PKFactoryNativeVideoPresentation?
  private var filePresentation: PKFactoryNativeFilePresentation?
  ...
  public func definition() -> ModuleDefinition {
    ...
    View(PKFactoryPresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: PKFactoryPresentationSourceView, identifier: String) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/PKFactoryShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("PKFactoryNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/PKFactoryShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "PKFactoryNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = PKFactoryNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func presentFile(url: URL, title: String, sourceIdentifier: String,
    ...
    let file = PKFactoryNativeFilePresentation(identifier: identifier, sources: presentationSources,
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "PKFactoryNativePresentation",
        code: 2,
