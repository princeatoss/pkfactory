...
final class PKFactoryNativeVideoPresentation: NSObject, AVPlayerViewControllerDelegate,
  ...
  func present(from presenter: UIViewController, sources: PKFactoryPresentationSources, sourceIdentifier: String) {
    let audioSession = AVAudioSession.sharedInstance()
    previousAudioSession = AudioSessionConfiguration(audioSession)
    do {
      try audioSession.setCategory(.playback, mode: .moviePlayback)
    } catch {
      NSLog("PK Factory video audio session: %@", error.localizedDescription)
    }
