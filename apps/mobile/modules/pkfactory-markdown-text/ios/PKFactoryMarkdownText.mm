...
#import "PKFactoryMarkdownTextShadowNode.h"
#import "PKFactoryMarkdownTextComponentDescriptor.h"
...
static void PKFactoryMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<PKFactoryMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void PKFactoryMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<PKFactoryMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = PKFactoryMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        PKFactoryMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol PKFactoryMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface PKFactoryMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<PKFactoryMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<PKFactoryMarkdownOutsideTapTarget>)target;

@end

static const void *PKFactoryMarkdownOutsideTapCoordinatorKey =
    &PKFactoryMarkdownOutsideTapCoordinatorKey;

@implementation PKFactoryMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<PKFactoryMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<PKFactoryMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, PKFactoryMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<PKFactoryMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static PKFactoryMarkdownOutsideTapCoordinator *
PKFactoryMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  PKFactoryMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, PKFactoryMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[PKFactoryMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        PKFactoryMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface PKFactoryMarkdownText () <RCTPKFactoryMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface PKFactoryMarkdownText () <PKFactoryMarkdownOutsideTapTarget>
@end

@implementation PKFactoryMarkdownText {
  UIView * _view;
  UITextView * _textView;
  PKFactoryMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<PKFactoryMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const PKFactoryMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    PKFactoryMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, PKFactoryMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [PKFactoryMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<PKFactoryMarkdownTextProps const>(_props);
  ...
  PKFactoryMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  PKFactoryMarkdownTextApplyAttachments(
      convertedAttrString,
      _state->getData().attachmentRanges,
      _attachmentImages);
  NSUInteger runLocation = 0;
  for (UIView *child in self.subviews) {
    if (![child isKindOfClass:[PKFactoryMarkdownTextRun class]]) {
      continue;
    }

    PKFactoryMarkdownTextRun *textChild = (PKFactoryMarkdownTextRun *)child;
    ...
    NSURL *link = [NSURL URLWithString:
        [NSString stringWithFormat:@"pkfactory-markdown-run://%ld", (long)textChild.tag]];
    if (link != nil) {
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::PKFactoryMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::PKFactoryMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<PKFactoryMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<PKFactoryMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<PKFactoryMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const PKFactoryMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable PKFactoryMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (PKFactoryMarkdownTextRun*)getTouchChild:(CGPoint)location
{
...
{
  PKFactoryMarkdownTextRun *child = [self childForCharacterRange:textItem.range];
  if (![child hasContextMenu]) {
    return defaultAction;
  }

  __weak PKFactoryMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::PKFactoryMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::PKFactoryMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> PKFactoryMarkdownTextCls(void)
{
  return PKFactoryMarkdownText.class;
}
