...
static constexpr auto FileAttachmentNativeIdPrefix = "pkfactory-file:";
static constexpr auto SkillAttachmentNativeIdPrefix = "pkfactory-skill:";
...
PKFactoryMarkdownTextShadowNode::PKFactoryMarkdownTextShadowNode(
   const ShadowNode& sourceShadowNode,
   const ShadowNodeFragment& fragment
) : ConcreteViewShadowNode(sourceShadowNode, fragment) {
};

Size PKFactoryMarkdownTextShadowNode::measureContent(
  ...
  const LayoutConstraints& layoutConstraints) const {
    ...
    auto paragraphStyleRanges = std::vector<PKFactoryMarkdownTextParagraphStyleRange>{};
    auto attachmentRanges = std::vector<PKFactoryMarkdownTextAttachmentRange>{};
    size_t utf16Offset = 0;
    const auto &children = getChildren();
    for (size_t i = 0; i < children.size(); i++) {
      const auto child = children[i].get();
      if (auto textViewChild = dynamic_cast<const PKFactoryMarkdownTextRunShadowNode *>(child)) {
        ...
        if (props.fontStyle == PKFactoryMarkdownTextRunFontStyle::Italic) {
          textAttributes.fontStyle = FontStyle::Italic;
        } else {
          textAttributes.fontStyle = FontStyle::Normal;
        }

        if (props.fontWeight == PKFactoryMarkdownTextRunFontWeight::Bold) {
          textAttributes.fontWeight = FontWeight::Bold;
        } else if (props.fontWeight == PKFactoryMarkdownTextRunFontWeight::UltraLight) {
          textAttributes.fontWeight = FontWeight::UltraLight;
        } else if (props.fontWeight == PKFactoryMarkdownTextRunFontWeight::Light) {
          textAttributes.fontWeight = FontWeight::Light;
        } else if (props.fontWeight == PKFactoryMarkdownTextRunFontWeight::Medium) {
          textAttributes.fontWeight = FontWeight::Medium;
        } else if (props.fontWeight == PKFactoryMarkdownTextRunFontWeight::Semibold) {
          textAttributes.fontWeight = FontWeight::Semibold;
        } else if (props.fontWeight == PKFactoryMarkdownTextRunFontWeight::Heavy) {
          textAttributes.fontWeight = FontWeight::Heavy;
        } else {
          textAttributes.fontWeight = FontWeight::Regular;
        }

        if (props.textDecorationLine == PKFactoryMarkdownTextRunTextDecorationLine::LineThrough) {
          textAttributes.textDecorationLineType = TextDecorationLineType::Strikethrough;
        } else if (props.textDecorationLine == PKFactoryMarkdownTextRunTextDecorationLine::Underline) {
        ...
        if (props.textDecorationStyle == PKFactoryMarkdownTextRunTextDecorationStyle::Solid) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Solid;
        } else if (props.textDecorationStyle == PKFactoryMarkdownTextRunTextDecorationStyle::Dotted) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Dotted;
        } else if (props.textDecorationStyle == PKFactoryMarkdownTextRunTextDecorationStyle::Dashed) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Dashed;
        } else if (props.textDecorationStyle == PKFactoryMarkdownTextRunTextDecorationStyle::Double) {
          textAttributes.textDecorationStyle = TextDecorationStyle::Double;
        }

        if (props.textAlign == PKFactoryMarkdownTextRunTextAlign::Left) {
          textAttributes.alignment = TextAlignment::Left;
        } else if (props.textAlign == PKFactoryMarkdownTextRunTextAlign::Right) {
          textAttributes.alignment = TextAlignment::Right;
        } else if (props.textAlign == PKFactoryMarkdownTextRunTextAlign::Center) {
          textAttributes.alignment = TextAlignment::Center;
        } else if (props.textAlign == PKFactoryMarkdownTextRunTextAlign::Justify) {
          textAttributes.alignment = TextAlignment::Justified;
        } else if (props.textAlign == PKFactoryMarkdownTextRunTextAlign::Auto) {
        ...
        if (hasParagraphStyle) {
          paragraphStyleRanges.push_back(PKFactoryMarkdownTextParagraphStyleRange{
              utf16Offset,
              fragmentLength,
        ...
        if (props.nativeId.rfind(FileAttachmentNativeIdPrefix, 0) == 0 && fragmentLength > 0) {
          attachmentRanges.push_back(PKFactoryMarkdownTextAttachmentRange{
              utf16Offset,
              1,
    ...
    if (baseProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Head) {
      textContainer.lineBreakMode = NSLineBreakByTruncatingHead;
    } else if (baseProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Middle) {
      textContainer.lineBreakMode = NSLineBreakByTruncatingMiddle;
    } else if (baseProps.ellipsizeMode == PKFactoryMarkdownTextEllipsizeMode::Tail) {
...
void PKFactoryMarkdownTextShadowNode::layout(LayoutContext layoutContext) {
  ensureUnsealed();
  setStateData(PKFactoryMarkdownTextStateReal{
    _attributedString,
