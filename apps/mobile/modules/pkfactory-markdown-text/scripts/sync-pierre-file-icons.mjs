...
const customSprite = webIconSource.match(/const PKFACTORY_FILE_ICON_SPRITE = `([\s\S]*?)`;/)?.[1];

if (!customSprite) {
  throw new Error(
    "Could not read the PK Factory Pierre icon sprite from apps/web/src/pierre-icons.ts",
  );
}
...
const customIcons = {
  agents: "pkfactory-file-icon-agents",
  claude: "pkfactory-file-icon-claude",
  package: "pkfactory-file-icon-package-json",
  pnpm: "pkfactory-file-icon-pnpm",
  readme: "pkfactory-file-icon-readme",
  tsconfig: "pkfactory-file-icon-tsconfig",
  video: "pkfactory-file-icon-video",
};
