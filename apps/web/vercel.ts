...
const ROUTER_HOST = "app.pkfactory.codes";
const HOSTED_WEB_CHANNEL_COOKIE = "pkfactory_web_channel";
const LATEST_ORIGIN = "https://latest.app.pkfactory.codes";
const NIGHTLY_ORIGIN = "https://nightly.app.pkfactory.codes";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @pkfactory/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --ignore-scripts --filter '@pkfactory/scripts...' --filter '@pkfactory/web...'",
  routes: [
    {
      src: "/__pkfactory/channel",
