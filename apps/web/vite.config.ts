...
import { DEV_PROXIED_PATH_PREFIXES } from "@pkfactory/shared/devProxy";
...
const isSingleOriginDev = process.env.PKFACTORY_SINGLE_ORIGIN_DEV === "1";
...
const configuredRelayUrl = repoEnv.VITE_PKFACTORY_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.PKFACTORY_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: PKFACTORY_BUNDLED_DEV=1 pnpm dev:web
// The dev runner defaults this on for --share runs (remote browsers pay a
// round trip per import level in unbundled dev); PKFACTORY_BUNDLED_DEV=0 opts out.
const bundledDevEnv = process.env.PKFACTORY_BUNDLED_DEV?.trim().toLowerCase();
...
function resolveDevProxyTarget(
  ...
  // PKFACTORY_PORT is set by scripts/dev-runner.ts for every non-desktop mode.
...
const devProxyTarget = resolveDevProxyTarget(process.env.PKFACTORY_PORT, configuredWsUrl);
...
function devCompressionPlugin(): Plugin {
  return {
    name: "pkfactory:dev-compression",
    apply: "serve",
...
const configuredAllowedHosts = (process.env.PKFACTORY_DEV_ALLOWED_HOSTS ?? "")
  .split(",")
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_PKFACTORY_RELAY_URL": JSON.stringify(configuredRelayUrl),
