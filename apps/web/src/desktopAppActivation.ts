  ...
  if (projectId === null) {
    try {
      ...
      return failure(
        ...
        errorMessage(error, "PK Factory could not add the project."),
      );
    }
  }
  ...
  try {
    ...
    if (opened === null) {
      return failure(
        request.requestId,
        "thread-open-failed",
        "PK Factory could not open a new thread for the project.",
      );
    }
    return {
      version: 1,
    ...
    return failure(
      request.requestId,
      "thread-open-failed",
      errorMessage(error, "PK Factory could not open a new thread for the project."),
    );
  }
}
