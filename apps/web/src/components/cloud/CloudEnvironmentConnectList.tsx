...
/**
 * The user's PK Factory Connect environments from relay discovery, each with a
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    if (result._tag === "Success") {
      toastManager.add({
        ...
        description: `Connecting to ${environment.label} through PK Factory Connect.`,
      });
      return;
    }
    ...
    const message =
      cause instanceof Error
        ? cause.message
        : "Could not connect the PK Factory Connect environment.";
    const traceId = findErrorTraceId(cause);
    console.error("[pkfactory-connect] Could not connect environment", { message, traceId, cause });
    toastManager.add({
      type: "error",
