import type { ServerProvider, ServerProviderVersionAdvisory } from "@pkfactory/contracts";
...
export function getProviderSummary(provider: ServerProvider | undefined) {
  ...
  if (!provider.enabled) {
    return {
      ...
      detail:
        provider.message ??
        "This provider is installed but disabled for new sessions in PK Factory.",
    };
  }
