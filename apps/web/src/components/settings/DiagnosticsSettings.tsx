        ...
        if (!result.value.signaled) {
          ...
          if (isStaleProcessSignalMessage(message)) {
            toastManager.add({
              ...
              description:
                "The process is not a child of the PK Factory Server. It might already have exited.",
            });
            return;
          }
      ...
      >
        <StatsGrid>
          <StatBlock
            ...
            tooltip="Approximate active CPU time for the PK Factory server root process and its descendants during the selected window. It grows only while sampled processes use CPU and older samples leave as the window moves."
          />
          <StatBlock
            label="Samples"
