...
function categoryLabel(category: ResourceTelemetryProcessCategory): string {
  switch (category) {
    ...
    case "resource-monitor":
      return "Monitor";
    case "unknown-pkfactory":
      return "PK Factory process";
  }
}
...
export function ResourceTelemetryDiagnostics() {
  ...
  const allPKFactory = snapshot?.groups.allPKFactory;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allPKFactory ? `${allPKFactory.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={
                allPKFactory
                  ? `${formatCpuTime(allPKFactory.cpuTimeMs)} observed CPU time`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allPKFactory ? formatBytes(allPKFactory.currentRssBytes) : "..."}
              detail={
                allPKFactory
                  ? `${formatBytes(allPKFactory.peakRssBytes)} combined process peaks`
                  : undefined
              }
            />
            <IconStat
              ...
              label="Process count"
              value={allPKFactory ? String(allPKFactory.processCount) : "..."}
              detail={
                allPKFactory
                  ? `${allPKFactory.processStarts} starts · ${allPKFactory.processExits} exits`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allPKFactory ? formatRate(allPKFactory.ioReadBytesPerSecond) : "..."}
              detail={
                allPKFactory ? `${formatBytes(allPKFactory.ioReadBytes)} observed` : undefined
              }
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allPKFactory ? formatRate(allPKFactory.ioWriteBytesPerSecond) : "..."}
              detail={
                allPKFactory ? `${formatBytes(allPKFactory.ioWriteBytes)} observed` : undefined
              }
              tone={
                allPKFactory && allPKFactory.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allPKFactory && allPKFactory.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known PK Factory operations so process spikes can be correlated with
