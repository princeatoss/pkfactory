import type { DesktopUpdateActionResult, DesktopUpdateState } from "@pkfactory/contracts";

export type DesktopUpdateButtonAction = "download" | "install" | "none";

const DESKTOP_RELEASE_HISTORY_URL = "https://github.com/princeatoss/pkfactory/releases";
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  if (action === "download") {
    return "This Mac has Apple Silicon, but PK Factory is still running the Intel build under Rosetta. Download the available update to switch to the native Apple Silicon build.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but PK Factory is still running the Intel build under Rosetta. Restart to install the downloaded Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but PK Factory is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateInstallConfirmationMessage(
  ...
  return `Install update${version ? ` ${version}` : ""} and restart PK Factory?\n\nAny running tasks will be interrupted. Make sure you're ready before continuing.`;
}
