...
/**
 * Read `defaultThreadEnvMode` from the project's checked-in `pkfactory.json`.
 *
 * Imperative counterpart to `usePKFactoryProjectFileScripts` for the new-thread
 ...
 * resolve to null.
 */
export async function readPKFactoryProjectFileDefaultThreadEnvMode(
  environmentId: EnvironmentId,
  workspaceRoot: string,
): Promise<ThreadEnvMode | null> {
  const result = await executeAtomQuery(
    appAtomRegistry,
    getProjectFileQueryAtom(environmentId, workspaceRoot, PKFACTORY_PROJECT_FILE_NAME),
  ...
  return parsePKFactoryProjectFile(data.contents)?.defaultThreadEnvMode ?? null;
}
