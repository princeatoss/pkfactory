...
import { ProjectId, type RepositoryIdentity } from "@pkfactory/contracts";
...
describe("matchesLinkedPullRequestUrl", () => {
  ...
  it("matches the same pull request without looking up its project", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/PingDotGG/PKFactory/pull/42/files",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a different pull request or host", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/princeatoss/pkfactory/pull/43",
      ),
    ).toBe(false);
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.example.com/princeatoss/pkfactory/pull/42",
      ),
    ).toBe(false);
  });
});
...
describe("openPullRequestLink", () => {
  it("opens the requested pull request URL", async () => {
    ...
    const targetUrl = "https://github.com/princeatoss/pkfactory/pull/123";
  ...
  it("reports bridge failures with a safe target origin", async () => {
    ...
    const targetUrl = "https://github.com/princeatoss/pkfactory/pull/123?token=secret";
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/PKFactoryTools/PKFactory/pull/123")).toEqual({
      host: "github.com",
      repository: "pkfactorytools/pkfactory",
      number: 123,
    });
  });
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl(
        "https://gitlab.com/pkfactorytools/platform/pkfactory/-/merge_requests/42",
      ),
    ).toEqual({
      host: "gitlab.com",
      repository: "pkfactorytools/platform/pkfactory",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/pkfactory/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/pkfactory",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/pkfactory/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/pkfactory",
      number: 17,
    });
  });
  ...
  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(
      parseChangeRequestUrl("https://github.com/pkfactorytools/pkfactory/pull/123/files?w=1"),
    ).toEqual({
      host: "github.com",
      repository: "pkfactorytools/pkfactory",
      number: 123,
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/pkfactorytools/pkfactory/issues/123",
      "https://github.com/pkfactorytools/pkfactory/commit/0a1b2c3",
      "https://github.com/pkfactorytools/pkfactory",
      "https://github.com/pkfactorytools/pkfactory/pull/abc",
      "https://gitlab.com/pkfactorytools/pkfactory/-/snippets/12",
      "https://gitlab.com/pkfactorytools/pkfactory/-/issues/12",
      ...
      "javascript:alert(1)//github.com/pkfactorytools/pkfactory/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `pkfactorytools/pkfactory` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/pkfactorytools/platform/pkfactory",
        provider: "gitlab",
        displayName: "pkfactorytools/platform/pkfactory",
  ...
  it("keeps two hosts apart, so an Enterprise link does not open the public one", () => {
    const projects = [
      project({
        canonicalKey: "github.com/princeatoss/pkfactory",
        provider: "github",
        owner: "pingdotgg",
        name: "pkfactory",
      }),
