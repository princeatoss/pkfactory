...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.pkfactory/worktrees/pkfactory-mvp/pkfactory-4e609bb8",
    );
    expect(result).toBe("pkfactory-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.pkfactory\\worktrees\\pkfactory-mvp\\pkfactory-4e609bb8",
    );
    expect(result).toBe("pkfactory-4e609bb8");
  });

  it("uses the final segment even when outside ~/.pkfactory/worktrees", () => {
