import { relayClerkTokenOptions } from "@pkfactory/shared/relayAuth";
...
export class CloudPublicConfigMissingError extends Schema.TaggedErrorClass<CloudPublicConfigMissingError>()(
  ...
  {
    key: Schema.Literal("PKFACTORY_CLERK_JWT_TEMPLATE"),
  },
) {
...
export function resolveCloudPublicConfig(): CloudPublicConfig {
  return {
    ...
    relayUrl: normalizeSecureRelayUrl(
      (import.meta.env.VITE_PKFACTORY_RELAY_URL as string | undefined) ?? "",
    ),
    relayTracing: {
...
export function resolveRelayClerkTokenOptions() {
  ...
  if (!clerkJwtTemplate) {
    throw new CloudPublicConfigMissingError({ key: "PKFACTORY_CLERK_JWT_TEMPLATE" });
  }
