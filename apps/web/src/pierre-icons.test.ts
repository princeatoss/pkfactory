...
describe("Pierre file icons", () => {
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(PKFACTORY_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(PKFACTORY_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
