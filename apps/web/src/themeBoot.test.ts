...
const THEME_STORAGE_KEY = "pkfactory:theme";
...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "PK Factory Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "pkfactory-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to PK Factory Chat",
      storage: {
        [THEME_STORAGE_KEY]: "pkfactory-chat",
    ...
    {
      name: "a legacy pkfactory-grove preference resolves through the alias",
      storage: {
        [THEME_STORAGE_KEY]: "pkfactory-grove",
        [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true",
      },
      prefersDark: true,
    },
    {
      name: "legacy pkfactory-chat-dark resolves to dark PK Factory Chat",
      storage: { [THEME_STORAGE_KEY]: "pkfactory-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("pkfactory-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "pkfactory:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("pkfactory-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(PKFACTORY_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "pkfactory-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "pkfactory:theme-halves:v1": JSON.stringify({ dark: "pkfactory-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "pkfactory-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "pkfactory:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("pkfactory-chat");
    expect(boot.isDark).toBe(true);
  });
