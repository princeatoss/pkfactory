import { DEFAULT_CLIENT_SETTINGS } from "@pkfactory/contracts";
...
describe("clientPersistenceStorage", () => {
  ...
  it("reports structured decode failures while preserving the fallback", async () => {
    ...
    testWindow.localStorage.setItem("pkfactory:client-settings:v1", "not-json");
    ...
    expect(consoleError).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        operation: "decode",
        storageKey: "pkfactory:client-settings:v1",
  ...
  it("keeps the diff layout across reloads and defaults it to stacked", async () => {
    ...
    expect(readBrowserClientSettings()).toBeNull();
    testWindow.localStorage.setItem("pkfactory:client-settings:v1", JSON.stringify({}));
