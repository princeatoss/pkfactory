...
describe("terminalLinkAtColumn", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://pkfactory.codes", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtColumn(row, 2)).toBe("https://pkfactory.codes");
    expect(terminalLinkAtColumn(row, cells.length - 1)).toBe("https://pkfactory.codes");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://pkfactory.codes", false, true)];
    expect(terminalLinkAtPosition(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://pkfactory.codes", false), row("", false)];
    expect(terminalLinkAtPosition(complete, 0, 8)).toBe("https://pkfactory.codes");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://pkfactory.code", (character) => cell(character)),
      ],
      text: "🙂 https://pkfactory.code",
      isWrapContinuation: false,
      wrapsToNext: true,
    };
    ...
    const unwrittenTail: GhosttyRow = {
      ...
      text: "https://pkfactory.codes",
      isWrapContinuation: false,
      wrapsToNext: false,
    };
    expect(terminalLinkAtPosition([unwrittenTail], 0, 8)).toBe("https://pkfactory.codes");
  });
});
