...
import {
  ...
  setupProjectScript,
} from "@pkfactory/shared/projectScripts";
...
describe("projectScripts helpers", () => {
  ...
  it("allows overriding runtime env values", () => {
    const env = projectScriptRuntimeEnv({
      project: { cwd: "/repo" },
      extraEnv: {
        PKFACTORY_PROJECT_ROOT: "/custom-root",
        CUSTOM_FLAG: "1",
      },
    });

    expect(env.PKFACTORY_PROJECT_ROOT).toBe("/custom-root");
    expect(env.CUSTOM_FLAG).toBe("1");
    expect(env.PKFACTORY_WORKTREE_PATH).toBeUndefined();
  });
