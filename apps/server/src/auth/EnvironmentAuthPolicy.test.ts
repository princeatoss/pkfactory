  ...
  EnvironmentAuthPolicy.layer.pipe(
    ...
    Layer.provide(
      Layer.effect(
        ...
        Layer.provide(
          ServerConfig.layerTest(process.cwd(), { prefix: "pkfactory-auth-policy-test-" }),
        ),
      ),
    ),
  );
...
it.layer(NodeServices.layer)("EnvironmentAuthPolicy.layer", (it) => {
  it.effect("uses desktop-managed-local policy for desktop mode", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.sessionCookieName).toBe("pkfactory_session_3773");
    }).pipe(
      Effect.provide(
  ...
  it.effect("keeps desktop cookies port-scoped on the port a second instance lands on", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.sessionCookieName).toBe("pkfactory_session_3774");
    }).pipe(
      Effect.provide(
        makeEnvironmentAuthPolicyLayer({
          mode: "desktop",
  ...
  it.effect("uses loopback-browser policy for loopback web hosts", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.sessionCookieName).toMatch(/^pkfactory_session_3773_[a-f0-9]{12}$/);
    }).pipe(
      Effect.provide(
        makeEnvironmentAuthPolicyLayer({
          mode: "web",
  ...
  it.effect("uses remote-reachable policy for wildcard web hosts", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.policy).toBe("remote-reachable");
      expect(descriptor.bootstrapMethods).toEqual(["one-time-token"]);
      expect(descriptor.sessionCookieName).toMatch(/^pkfactory_session_[a-f0-9]{12}$/);
    }).pipe(
      Effect.provide(
        makeEnvironmentAuthPolicyLayer({
          mode: "web",
  ...
  it.effect("isolates wildcard-bound web development sessions", () =>
    Effect.gen(function* () {
      ...
      expect(descriptor.policy).toBe("remote-reachable");
      expect(descriptor.sessionCookieName).toMatch(/^pkfactory_session_5775_[a-f0-9]{12}$/);
    }).pipe(
      Effect.provide(
        makeEnvironmentAuthPolicyLayer({
          mode: "web",
