...
import type { CursorSettings } from "@pkfactory/contracts";
...
const cursorAcpDiscoveryFailedMessage = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart PK Factory, and try again.",
  "See https://cursor.com/docs/cli/installation.",
  "Check server logs for ACP details.",
].join(" ");
const missingCursorBinaryPath = "/definitely/not/installed/pkfactory-cursor-agent";
const cursorCliCommandMissingMessage = [
  `Cursor CLI command \`${missingCursorBinaryPath}\` was not found.`,
  `Install or enable the Cursor CLI, make sure \`${missingCursorBinaryPath}\` is on PATH, then restart PK Factory.`,
  "See https://cursor.com/docs/cli/installation.",
].join(" ");
