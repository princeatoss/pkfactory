...
class CursorAdapter extends Context.Service<CursorAdapter, CursorAdapterShape>()(
  "pkfactory/provider/Layers/CursorAdapter.test/CursorAdapter",
) {}
...
export PKFACTORY_ACP_REQUEST_LOG_PATH=${JSON.stringify(requestLogPath)}
${envExports}
exec ${JSON.stringify(mockAgentCommand)} ${mockAgentArgs.map((arg) => JSON.stringify(arg)).join(" ")} "$@"
`;
  await NodeFSP.writeFile(wrapperPath, script, "utf8");
  ...
  Layer.effect(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "pkfactory-cursor-adapter-test-",
      }),
    ),
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
  ...
  it.effect("steers a running turn instead of opening a new one on mid-turn sendTurn", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ PKFACTORY_ACP_PROMPT_DELAY_MS: "1500" }),
      );
      yield* settings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const previousEmitToolCalls = process.env.PKFACTORY_ACP_EMIT_TOOL_CALLS;
        process.env.PKFACTORY_ACP_EMIT_TOOL_CALLS = "1";
        ...
        const wrapperPath = yield* Effect.promise(() =>
          makeMockAgentWrapper({ PKFACTORY_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
          ...
          Effect.ensuring(
            Effect.sync(() => {
              if (previousEmitToolCalls === undefined) {
                delete process.env.PKFACTORY_ACP_EMIT_TOOL_CALLS;
              } else {
                process.env.PKFACTORY_ACP_EMIT_TOOL_CALLS = previousEmitToolCalls;
              }
            }),
          ),
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => NodeFSP.writeFile(requestLogPath, "", "utf8"));
        const wrapperPath = yield* Effect.promise(() =>
          makeProbeWrapper(requestLogPath, argvLogPath, { PKFACTORY_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
  ...
  it.effect("segments assistant messages around ACP tool activity in full-access mode", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ PKFACTORY_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1" }),
      );
      yield* serverSettings.updateSettings({
        providers: { cursor: { binaryPath: wrapperPath } },
      });
  ...
  it.effect("stopping a session settles pending user-input waits", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ PKFACTORY_ACP_EMIT_ASK_QUESTION: "1" }),
      );
      yield* serverSettings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () => {
      ...
      const customAdapterLayer = Layer.effect(
        ...
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "pkfactory-cursor-adapter-custom-instance-",
          }),
        ),
        Layer.provideMerge(NodeServices.layer),
      );
