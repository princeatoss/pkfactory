...
const grokAdapterTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "pkfactory-grok-adapter-test-",
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  it.effect("sends runtime context with the current model without changing saved prompts", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ PKFACTORY_ACP_REQUEST_LOG_PATH: requestLogPath }),
      );
  ...
  it.effect("closes the ACP child process when a session stops", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EXIT_LOG_PATH: exitLogPath,
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("reports a Grok session running only while the prompt is in flight", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_TOOL_CALLS: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const requestOpened =
  ...
  it.effect("completes a Grok turn from xAI prompt completion when the prompt RPC hangs", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
  ...
  it.effect("does not time out a Grok turn before ACP emits progress", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_HANG_PROMPT_FOREVER: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath, {
  ...
  it.effect("fails a Grok turn that stalls after ACP content begins", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_CONTENT_THEN_HANG: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath, {
        turnInactivityTimeoutMs: 1_000,
  ...
  it.effect("refreshes Grok liveness when ACP updates its plan", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_PLAN_THEN_HANG: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath, {
        turnInactivityTimeoutMs: 1_000,
  ...
  it.effect("settles a stalled Grok turn after the active-tool deadline", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_ACTIVE_TOOL_THEN_HANG: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath, {
        turnInactivityTimeoutMs: 1_000,
  ...
  it.effect("does not report a synthetic stop reason when xAI omits one", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
          PKFACTORY_ACP_OMIT_XAI_PROMPT_COMPLETE_STOP_REASON: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("drops late ACP notifications after a turn is cancelled", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_HANG_PROMPT_FOREVER: "1",
          PKFACTORY_ACP_EMIT_LATE_UPDATE_AFTER_CANCEL: "1",
        }),
      );
  ...
  it.effect("surfaces Grok usage limits without clearing the selected model", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_XAI_RATE_LIMIT_THEN_HANG: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const runtimeEvents: ProviderRuntimeEvent[] = [];
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_REQUEST_LOG_PATH: requestLogPath,
          PKFACTORY_ACP_EMIT_TOOL_CALLS: "1",
          PKFACTORY_ACP_ALLOW_ONCE_OPTION_ID: "agent-defined-approval-id",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const eventsFiber = yield* Stream.runForEach(adapter.streamEvents, (event) =>
  ...
  it.effect("captures xAI exit_plan_mode as a proposed plan and unblocks the turn", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ PKFACTORY_ACP_EMIT_XAI_EXIT_PLAN_MODE: "1" }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const proposed =
  ...
  it.effect("surfaces plan.md writes as a proposed plan while plan mode is active", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ PKFACTORY_ACP_EMIT_XAI_PLAN_MD_WRITE: "1" }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const proposed =
        yield* Deferred.make<Extract<ProviderRuntimeEvent, { type: "turn.proposed.completed" }>>();
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_REQUEST_LOG_PATH: requestLogPath,
          PKFACTORY_ACP_EMIT_TOOL_CALLS: "1",
          PKFACTORY_ACP_OMIT_ALLOW_ALWAYS: "1",
          PKFACTORY_ACP_PERMISSION_REQUEST_COUNT: "2",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          ...
          PKFACTORY_ACP_OMIT_ALLOW_ALWAYS: "1",
          PKFACTORY_ACP_PERMISSION_REQUEST_COUNT: "2",
          PKFACTORY_ACP_PERMISSION_TITLE: "Terminal",
          PKFACTORY_ACP_SECOND_PERMISSION_COMMAND: "rm server/package.json",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const openedCount = yield* Ref.make(0);
  ...
  it.effect("captures a plan under the provider instance GROK_HOME", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          PKFACTORY_ACP_EMIT_XAI_PLAN_MD_WRITE: "1",
          PKFACTORY_ACP_PLAN_ROOT: grokHome,
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath, {
  ...
  it.effect("handles xAI ask_user_question extension requests", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ PKFACTORY_ACP_EMIT_XAI_ASK_USER_QUESTION: "1" }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const requested =
  ...
  it.effect("settles a stalled Grok turn after its first activity is user input", () =>
    Effect.gen(function* () {
      const threadId = ThreadId.make("grok-xai-ask-user-question");
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ PKFACTORY_ACP_EMIT_XAI_ASK_USER_QUESTION_THEN_HANG: "1" }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath, {
        turnInactivityTimeoutMs: 1_000,
      });
