import { ClaudeSettings } from "@pkfactory/contracts";
...
it.layer(NodeServices.layer)("Claude capability probe SDK boundary", (it) => {
  it.effect("serializes strict no-MCP options and still resolves account capabilities", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-claude-probe-sdk-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          "}",
          "writeFileSync(process.env.PKFACTORY_PROBE_INVOCATION_PATH, JSON.stringify({",
          "  args,",
          "  cwd: process.cwd(),",
      ...
      const capabilities = yield* probeClaudeCapabilities(
        ...
        {
          ...process.env,
          PKFACTORY_PROBE_INVOCATION_PATH: invocationPath,
          ENABLE_CLAUDEAI_MCP_SERVERS: "true",
        },
        workspaceCwd,
      );
