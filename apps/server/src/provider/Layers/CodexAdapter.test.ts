...
class CodexAdapter extends Context.Service<CodexAdapter, CodexAdapterShape>()(
  "pkfactory/provider/Layers/CodexAdapter.test/CodexAdapter",
) {}
...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  ...
  it.effect("uses PKFACTORY_CODEX_LAUNCH_ARGS for the session runtime", () => {
    const runtimeFactory = makeRuntimeFactory();
    const layer = Layer.effect(
      CodexAdapter,
      Effect.gen(function* () {
        ...
        return yield* makeCodexAdapter(codexConfig, {
          environment: { PKFACTORY_CODEX_LAUNCH_ARGS: " --strict-config --enable env-feature " },
          makeRuntime: runtimeFactory.factory,
        });
      }),
    ).pipe(
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("labels MCP lifecycle entries with server and tool names", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(firstEvent.value.payload.title, "pkfactory · preview_status");
...
it.effect("flushes managed native logs when the adapter layer shuts down", () =>
  Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "pkfactoryx-adapter-native-log-"),
    );
