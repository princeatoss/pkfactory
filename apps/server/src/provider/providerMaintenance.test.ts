...
import { ProviderDriverKind, ProviderInstanceId, type ServerProvider } from "@pkfactory/contracts";
...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("pkfactory-vite-plus-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("pkfactory-bun-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("pkfactory-pnpm-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("pkfactory-native-package-tool-native-capabilities");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("pkfactory-scoped-package-tool-native-capabilities");
  ...
  it.effect("keeps npm updates for binaries symlinked into npm's global node_modules tree", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("pkfactory-npm-capabilities");
  ...
  it.effect("uses Effect FileSystem realPath when detecting pnpm global symlinks", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("pkfactory-pnpm-realpath-capabilities");
      const binDir = NodePath.join(tempDir, "bin");
      const packageBinDir = NodePath.join(
        tempDir,
        ".local",
