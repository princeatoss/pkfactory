...
// PKFACTORY_CODEX_COLLAB_SCRIPT env var (a JSON file path) when the first turn
...
const script = JSON.parse(NodeFS.readFileSync(process.env.PKFACTORY_CODEX_COLLAB_SCRIPT, "utf8"));
...
rl.on("line", (line) => {
  ...
  if (method === undefined && script.serverRequests?.some((request) => request.id === id)) {
    NodeFS.appendFileSync(
      `${process.env.PKFACTORY_CODEX_COLLAB_SCRIPT}.responses`,
  ...
  if (method === "initialize") {
    write({
      id,
      result: {
        userAgent: "pkfactory-collab-mock/0.0.0",
        codexHome: "/tmp",
        platformFamily: "unix",
        platformOs: "linux",
      },
  ...
  if (method === "thread/resume") {
    if (script.recordRequests) {
      NodeFS.appendFileSync(
        `${process.env.PKFACTORY_CODEX_COLLAB_SCRIPT}.requests`,
  ...
  if (method === "turn/interrupt") {
    ...
    NodeFS.appendFileSync(
      `${process.env.PKFACTORY_CODEX_COLLAB_SCRIPT}.interrupts`,
