...
/**
 * A pinned runtime is an exact `pkfactory@<version>` npm-installed into
 ...
 * switching over, never `npx pkfactory`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "pkfactory", "dist", "bin.mjs"),
...
/**
 * Installs `pkfactory@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "pkfactory", "dist", "bin.mjs"),
    sentinelPath: input.path.join(stagingDir, ".install-complete"),
  };

  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned pkfactory runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
