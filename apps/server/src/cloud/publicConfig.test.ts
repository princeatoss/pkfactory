...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ PKFACTORY_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ PKFACTORY_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ PKFACTORY_HOSTED_APP_URL: "https://nightly.app.pkfactory.codes" }),
      ),
      "https://nightly.app.pkfactory.codes",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ PKFACTORY_HOSTED_APP_URL: "http://localhost:5733" }),
      ),
      "http://localhost:5733",
    );
  }),
...
it.effect("rejects malformed or insecure hosted app URLs", () =>
  Effect.gen(function* () {
    for (const value of [
      "app.pkfactory.codes",
      "http://app.pkfactory.codes",
      "https://app.pkfactory.codes/nested",
      "https://app.pkfactory.codes?alias=true",
    ]) {
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ PKFACTORY_HOSTED_APP_URL: value }),
        Effect.result,
      );
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        PKFACTORY_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
