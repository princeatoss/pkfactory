...
import type { RelayLinkProofRequest } from "@pkfactory/contracts/relay";
  ...
  it.effect("does not accept an unauthenticated request trace parent", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("https://environment.example.test/api/pkfactory-cloud/mint-credential", {
          headers: {
...
describe("releaseManagedTunnelOnShutdown", () => {
  ...
  const provideReleaseHarness =
    ...
    <A, E, R>(effect: Effect.Effect<A, E, R>) =>
      effect.pipe(
        ...
        Effect.provide(
          ServerConfigModule.layerTest("/", { prefix: "pkfactory-http-release-test-" }).pipe(
  ...
  it.effect("still releases a pending update when the launcher is stopping", () => {
    // `pkfactory service uninstall` or `systemctl stop` during the pending window:
...
describe("link proof provider kinds", () => {
  ...
  it("accepts managed and manual endpoints but not pkfactory_relay", () => {
    ...
    expect(isSupportedLinkProviderKind(proofRequest("pkfactory_relay"))).toBe(false);
  });
