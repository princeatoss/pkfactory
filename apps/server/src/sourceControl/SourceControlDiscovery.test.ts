...
import { VcsProcessSpawnError } from "@pkfactory/contracts";
    ...
    Layer.provide(
      Layer.mergeAll(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "pkfactory-source-control-registry-test-",
...
it.effect("reports implemented tools separately from locally available executables", () => {
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "pkfactory-source-control-discovery-",
      }),
    ),
...
`),
  ...
  const testLayer = SourceControlDiscovery.layer.pipe(
    Layer.provide(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "pkfactory-source-control-auth-discovery-",
      }),
    ),
    Layer.provide(Layer.mock(VcsProcess.VcsProcess)(processMock)),
    Layer.provide(
