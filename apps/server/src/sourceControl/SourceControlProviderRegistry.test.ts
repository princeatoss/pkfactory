...
import { VcsRepositoryDetectionError } from "@pkfactory/contracts";
...
it.effect("routes GitHub remotes to the GitHub provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@github.com:princeatoss/pkfactory.git" }],
    });
...
it.effect("routes Bitbucket remotes to the Bitbucket provider", () =>
  Effect.gen(function* () {
    const registry = yield* makeRegistry({
      remotes: [{ name: "origin", url: "git@bitbucket.org:princeatoss/pkfactory.git" }],
    });
