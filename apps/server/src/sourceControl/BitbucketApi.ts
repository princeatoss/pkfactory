...
import { sanitizeBranchFragment } from "@pkfactory/shared/git";
import {
  detectSourceControlProviderFromRemoteUrl,
  isSshRemoteUrl,
} from "@pkfactory/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("PKFACTORY_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("PKFACTORY_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("PKFACTORY_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("PKFACTORY_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("pkfactory/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `pkfactory/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set PKFACTORY_BITBUCKET_EMAIL and PKFACTORY_BITBUCKET_API_TOKEN, or PKFACTORY_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
