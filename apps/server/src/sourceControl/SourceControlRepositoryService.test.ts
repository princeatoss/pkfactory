...
import { GitCommandError, SourceControlProviderError } from "@pkfactory/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/pkfactory",
  sshUrl: "git@github.com:octocat/pkfactory.git",
};
  ...
  const serviceLayer = SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(
      ServerConfig.layerTest(
        process.cwd(),
        input.fileSystem
          ? "/tmp/pkfactory-source-control-repos"
          : { prefix: "pkfactory-source-control-repos-" },
      ),
    ),
  );
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/pkfactory" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "pkfactory-source-control-clone-parent-",
    });
    const destinationPath = `${parent}/pkfactory`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "pkfactory"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("preserves destination probe failures instead of treating them as missing paths", () => {
  const fileSystemCause = PlatformError.systemError({
    ...
    module: "FileSystem",
    method: "exists",
    pathOrDescriptor: "/restricted/pkfactory",
  });
  ...
  return Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.cloneRepository({
        ...
        destinationPath: "/restricted/pkfactory",
      }),
    );
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/pkfactory", visibility: "private" },
    ]);
