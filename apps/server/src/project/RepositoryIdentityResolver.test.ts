...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
  it.effect("reuses the cached Git root for repeated workspace lookups", () => {
        ...
        Effect.sync(() => {
          ...
          return {
            stdout: input.args.includes("rev-parse")
              ? "/repo\n"
              : "origin\tgit@github.com:PKFactoryTools/pkfactory.git (fetch)\n",
            stderr: "",
    ...
    return Effect.gen(function* () {
      ...
      expect(first?.canonicalKey).toBe("github.com/pkfactorytools/pkfactory");
  ...
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:PKFactoryTools/pkfactory.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/pkfactorytools/pkfactory");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("pkfactorytools/pkfactory");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("pkfactorytools");
      expect(identity?.name).toBe("pkfactory");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, [
        ...
        "add",
        "origin",
        "git@github.com:PKFactoryTools/pkfactory.git",
      ]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-no-remote-",
      });
  ...
  it.effect("prefers upstream over origin when both remotes are configured", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-upstream-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/pkfactory.git"]);
      yield* git(cwd, ["remote", "add", "upstream", "git@github.com:PKFactoryTools/pkfactory.git"]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-nested-group-test-",
      });
      ...
      yield* git(cwd, [
        ...
        "add",
        "origin",
        "git@gitlab.com:PKFactoryTools/platform/pkfactory.git",
      ]);
      ...
      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/pkfactorytools/platform/pkfactory");
      expect(identity?.displayName).toBe("pkfactorytools/platform/pkfactory");
      expect(identity?.owner).toBe("pkfactorytools");
      expect(identity?.name).toBe("pkfactory");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "pkfactory-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/pkfactorytools/pkfactory");
        expect(refreshedIdentity?.name).toBe("pkfactory");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/pkfactorytools/pkfactory");

      yield* git(cwd, [
        "remote",
        "set-url",
        "origin",
        "git@github.com:PKFactoryTools/pkfactory-next.git",
      ]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/pkfactorytools/pkfactory");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/pkfactorytools/pkfactory-next");
      expect(refreshedIdentity?.displayName).toBe("pkfactorytools/pkfactory-next");
      expect(refreshedIdentity?.name).toBe("pkfactory-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
