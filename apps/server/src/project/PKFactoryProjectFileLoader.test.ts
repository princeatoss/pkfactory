...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(PKFactoryProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "pkfactory.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("PKFactoryProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid pkfactory.json", () =>
      Effect.gen(function* () {
        const loader = yield* PKFactoryProjectFileLoader.PKFactoryProjectFileLoader;
    ...
    it.effect("returns none when pkfactory.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* PKFactoryProjectFileLoader.PKFactoryProjectFileLoader;
        const cwd = yield* makeTempDir;
