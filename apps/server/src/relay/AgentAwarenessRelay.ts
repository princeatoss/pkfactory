...
import { projectThreadAwareness } from "@pkfactory/shared/agentAwareness";
...
>()("pkfactory/relay/AgentAwarenessRelay") {}
...
const makePublishProof = Effect.fn("makePublishProof")(function* (input: {
  ...
  const payload = {
    iss: `pkfactory-env:${input.environmentId}`,
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for PK Factory Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
            "agent activity publishing disabled by PK Factory Connect configuration",
          );
          break;
        case "enabled":
