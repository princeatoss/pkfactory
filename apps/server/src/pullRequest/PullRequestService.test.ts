...
it.effect("reads nothing from a host with no implementation, but reports it", () =>
  Effect.gen(function* () {
    ...
    assert.deepStrictEqual(listed, ["princeatoss/pkfactory"]);
...
it.effect("says where each repository carries on, and from nothing it has run out of", () =>
  Effect.gen(function* () {
      ...
      providers: [
        fakeProvider("github", {
          listChangeRequests: ({ repository }) =>
            Effect.succeed({
              items: [changeRequest(1, "2026-07-02T00:00:00Z")],
              truncated: repository === "princeatoss/pkfactory",
              continues: true,
            }),
        }),
      ],
    ...
    assert.deepStrictEqual(result.nextCursors, {
      "github.com princeatoss/pkfactory": "2026-07-02T00:00:00Z|1|1",
    });
  }),
);
...
it.effect("keeps a row already sent at the boundary instant from arriving twice", () =>
  Effect.gen(function* () {
    ...
    const result = yield* service.list({
      state: "open",
      cursors: { "github.com princeatoss/pkfactory": "2026-07-02T00:00:00Z|1|7" },
    });
    ...
    assert.deepStrictEqual(result.nextCursors, {
      "github.com princeatoss/pkfactory": "2026-07-01T00:00:00Z|3|9",
    });
  }),
);
...
it.effect("keeps the earlier exclusions when a slice ends on the instant it began on", () =>
  Effect.gen(function* () {
    ...
    const result = yield* service.list({
      state: "open",
      cursors: { "github.com princeatoss/pkfactory": "2026-07-02T00:00:00Z|1|6" },
    });
    ...
    assert.deepStrictEqual(result.nextCursors, {
      "github.com princeatoss/pkfactory": "2026-07-02T00:00:00Z|3|6,7,8",
    });
  }),
);
...
it.effect("refuses a continuation it did not issue, before asking any host anything", () =>
  Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.list({ state: "open", cursors: { "github.com princeatoss/pkfactory": "yesterday" } }),
    );
  ...
  Effect.gen(function* () {
    ...
    const service = yield* makeService({
      projects: [
        ...
        project({
          id: "p2",
          title: "pkfactory worktree",
          workspaceRoot: "/b",
          repository: "PingDotGG/PKFactory",
        }),
      ],
      providers: [
        fakeProvider("github", {
  ...
  Effect.gen(function* () {
    ...
    const service = yield* makeService({
      projects: [
        ...
        project({
          id: "p1",
          title: "pkfactory (stale worktree)",
          workspaceRoot: "/gone",
          repository: "princeatoss/pkfactory",
        }),
        project({
...
it.effect("reads a host's repositories in one search, and files the rows back under each", () =>
  Effect.gen(function* () {
          ...
          listChangeRequestsAcross: (input) => {
            ...
            return Effect.succeed({
              items: [
                ...
                batchedChangeRequest(2, "princeatoss/pkfactory", "2026-07-02T00:00:00Z"),
              ],
              truncated: false,
            });
          },
    ...
    assert.deepStrictEqual(asked, [["princeatoss/pkfactory", "acme/web"]]);
...
it.effect("carries every repository of a slice on from the oldest row in it", () =>
  Effect.gen(function* () {
    ...
    assert.isTrue(result.truncated);
    assert.deepStrictEqual(result.nextCursors, {
      "github.com princeatoss/pkfactory": "2026-07-02T00:00:00Z|1|2",
