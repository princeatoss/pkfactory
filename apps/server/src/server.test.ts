...
import { HostProcessEnvironment, HostProcessPlatform } from "@pkfactory/shared/hostProcess";
...
import {
  ...
  type DpopPublicJwk,
} from "@pkfactory/shared/dpop";
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@pkfactory/shared/relayJwt";
import * as RelayClient from "@pkfactory/shared/relayClient";
  ...
  Effect.gen(function* () {
    ...
    yield* Layer.build(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "pkfactory-web",
          attributes: {
            "service.runtime": "pkfactory-web",
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected PK Factory Connect CLI authorization request.")),
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `pkfactory-env:${input.environmentId}`,
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("serves static index content for GET / when staticDir is configured", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-router-static-",
      });
  ...
  it.effect("caches hashed static assets without freezing mutable files or SPA fallbacks", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-static-hashes-",
      });
  ...
  for (const manifest of [
    ...
    it.effect(`revalidates hash-like static filenames with a ${manifest.label} manifest`, () =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const staticDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "pkfactory-static-mutable-",
        });
        yield* fileSystem.makeDirectory(path.join(staticDir, "assets"));
  ...
  it.effect("binds static metadata and bytes to one file across atomic replacement", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-static-replace-",
      });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/pkfactory/environment");
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("pkfactory_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("migrates a valid legacy remote-web session cookie", () =>
    Effect.gen(function* () {
      ...
      const legacyCookie = currentCookie.replace(/^pkfactory_session_[^=]+=/, "pkfactory_session=");
      const sessionUrl = yield* getHttpServerUrl("/api/auth/session");
    ...
    (source) =>
      Effect.gen(function* () {
        ...
        const response = yield* fetchEffect(sessionUrl, {
          headers:
            source === "cookie"
              ? { cookie: `${sessionCookie}; pkfactory_session=stale` }
              : { authorization: `Bearer ${sessionToken}`, cookie: "pkfactory_session=stale" },
        });
        const body = yield* responseJsonEffect<{ readonly authenticated: boolean }>(response);
      ...
      Effect.gen(function* () {
        ...
        const tokenResponse = yield* fetchEffect(tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            subject_token_type: "urn:pkfactory:params:oauth:token-type:environment-bootstrap",
      ...
      const linkProofResponse = yield* fetchEffect(linkProofUrl, {
        ...
        body: jsonRequestBody({
          ...
          endpoint: {
            ...
            // "manual" and "cloudflare_tunnel" are supported; "pkfactory_relay" is not.
            providerKind: "pkfactory_relay",
          },
          origin: {
    ...
    () =>
      Effect.gen(function* () {
        const installedRelayClient = {
          ...
          executablePath: "/tmp/pkfactory/tools/cloudflared",
    ...
    Effect.gen(function* () {
      ...
      const relayConfigResponse = yield* fetchEffect(relayConfigUrl, {
        ...
        body: jsonRequestBody({
          ...
          environmentCredential: "pkfactoryenv_test_credential",
  ...
  it.effect("rejects relay config replacement from a different cloud account", () =>
    Effect.gen(function* () {
      ...
      const firstResponse = yield* postRelayConfig("user_123", "pkfactoryenv_first_credential");
      const replacementResponse = yield* postRelayConfig(
        "user_456",
        "pkfactoryenv_second_credential",
      );
  ...
  it.effect("serves the documented PK Factory Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const mintUrl = yield* getHttpServerUrl("/api/pkfactory-connect/mint-credential");
  ...
  it.effect("serves signed PK Factory Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const healthUrl = yield* getHttpServerUrl("/api/pkfactory-connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "pkfactory-env:other-environment",
  ...
  for (const desktopOrigin of ["pkfactory://app", "pkfactory-dev://app"]) {
        ...
        resourceSpans: [
          {
            resource: {
              attributes: [
                {
                  key: "service.name",
                  value: { stringValue: "pkfactory-web" },
                },
              ],
            },
            scopeSpans: [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "pkfactory-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-auth-required-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-project-search-",
      });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
  ...
  it.effect("routes websocket rpc projects.listEntries and projects.readFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-project-files-",
      });
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-project-search-gitignored-",
      });
  ...
  it.effect("preserves structured workspace rpc failures", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-workspace-errors-",
      });
      const outsideDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-workspace-errors-outside-",
      });
  ...
  it.effect("reports workspace root stat failures without relabeling them as missing", () =>
    Effect.gen(function* () {
      ...
      const blockedRoot = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-workspace-stat-error-",
      });
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-project-write-",
      });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      ...
      const parentDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ws-project-create-",
      });
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "pkfactory/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "pkfactory/bootstrap-refName",
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          ...
          newRefName: "pkfactory/bootstrap-refName",
          baseRefName: "main",
          path: null,
        });
...
it.live(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const reportPath = yield* Config.string("PKFACTORY_TRANSFER_BUDGET_REPORT_PATH").pipe(
        Config.option,
      );
      ...
      const resultPath = yield* Config.string("PKFACTORY_TRANSFER_BUDGET_RESULT_PATH").pipe(
        Config.option,
      );
