...
import type { DesktopAppActivationRequest } from "@pkfactory/contracts";
...
describe("pkfactory app", () => {
  it.effect("rejects SSH before it tries to reach a desktop app", () =>
    withTempDirectory("pkfactory-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-pkfactory-home");
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`pkfactory app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
  ...
  it.effect("rejects unsupported platforms without creating state", () =>
    withTempDirectory("pkfactory-app-platform-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-pkfactory-home");
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`pkfactory app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
  ...
  it.effect("does not create state when only a server or no desktop app is running", () =>
    withTempDirectory("pkfactory-app-missing-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "missing-pkfactory-home");
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the PK Factory desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
  ...
  it.effect("uses PKFACTORY_HOME or --base-dir and sends the default or explicit path", () =>
    withTempDirectory("pkfactory-app-command-test-", (root) =>
      Effect.gen(function* () {
        const baseDir = NodePath.join(root, "pkfactory-home");
        ...
        yield* runCli(["app"], { PKFACTORY_HOME: baseDir });
  ...
  it.effect("prefers the installed desktop app when a dev desktop is also running", () =>
    withTempDirectory("pkfactory-app-preferred-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".pkfactory");
        const desktop = yield* fakeDesktop({ baseDir });
  ...
  it.effect("finds the dev desktop when the default desktop socket is absent", () =>
    withTempDirectory("pkfactory-app-dev-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".pkfactory");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        yield* runCli(["app"]);
        yield* runCli(["app"], { PKFACTORY_HOME: "   " });
  ...
  it.effect("never searches a dev state directory for an explicit PK Factory home", () =>
    withTempDirectory("pkfactory-app-explicit-test-", (root) =>
      Effect.gen(function* () {
        vi.mocked(NodeOS.homedir).mockReturnValue(root);
        const baseDir = NodePath.join(root, ".pkfactory");
        const development = yield* fakeDesktop({ baseDir, stateSubdirectory: "dev" });
        ...
        const envError = yield* runCli(["app"], { PKFACTORY_HOME: baseDir }).pipe(Effect.flip);
  ...
  for (const responseKind of ["failure", "invalid"] as const) {
    it.effect(`never falls back after the default desktop sends a ${responseKind} response`, () =>
      withTempDirectory("pkfactory-app-response-test-", (root) =>
        Effect.gen(function* () {
          vi.mocked(NodeOS.homedir).mockReturnValue(root);
          const baseDir = NodePath.join(root, ".pkfactory");
