  ...
  Effect.acquireUseRelease(
    Effect.callback<NodeHttp.Server>((resume) => {
      const server = NodeHttp.createServer((request, response) => {
        if (request.url === "/.well-known/pkfactory/environment") {
...
describe("pkfactory pair", () => {
  it.effect("mints a token and prints a QR pairing URL for a live server", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "pkfactory-pair-test-"));
        ...
        assert.equal(credentials[0]?.label, "pkfactory pair");
      }),
    ).pipe(
    ...
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "pkfactory-pair-dev-test-"),
        );
        const port = Number(new URL(origin).port);
  ...
  it.effect("directs to pkfactory serve or pkfactory connect when no server is running", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "pkfactory-pair-none-test-"),
      );
      ...
      assert.include(rendered, "No running PK Factory server found.");
      assert.include(rendered, "npx pkfactory serve");
      assert.include(rendered, "npx pkfactory connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("ignores runtime state whose recorded pid is no longer alive", () =>
    withDescriptorServer((origin) =>
      Effect.gen(function* () {
        const baseDir = NodeFS.mkdtempSync(
          NodePath.join(NodeOS.tmpdir(), "pkfactory-pair-pid-test-"),
        );
        const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
  ...
  it.effect("ignores stale runtime state pointing at a dead server", () =>
    Effect.gen(function* () {
      const baseDir = NodeFS.mkdtempSync(
        NodePath.join(NodeOS.tmpdir(), "pkfactory-pair-stale-test-"),
      );
      const statePath = NodePath.join(baseDir, "userdata", "server-runtime.json");
