...
import { HostProcessArguments } from "@pkfactory/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `pkfactory` command.
 */
...
/**
 * The `pkfactory` package spec to suggest. The literal spec the user typed (e.g.
 * `pkfactory@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "pkfactory@nightly" : "pkfactory";
}

/**
 * Render a `pkfactory <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx pkfactory connect` suggests
 * `npx pkfactory serve`, a global install suggests `pkfactory serve`, and a nightly build
...
export function formatCliCommand(input: {
  ...
  if (runner === null) {
    return `pkfactory ${input.subcommand}`;
  }
