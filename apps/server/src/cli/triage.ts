/**
 * `pkfactory triage` - hand a misbehaving install to the user's own coding agent.
 *
...
import { isCommandAvailable, resolveSpawnCommand } from "@pkfactory/shared/shell";
...
export const triageCommand = Command.make("triage", {
  ...
  Command.withDescription(
    "Investigate a PK Factory problem on this machine with claude or codex, and help file a good issue.",
  ),
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // --base-dir wins; PKFACTORY_HOME is its documented env equivalent (same
      // precedence as `pkfactory pair`).
