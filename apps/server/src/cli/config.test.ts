...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  pkfactoryHome: "/tmp/pkfactory-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "pkfactory-cli-config-env-base");
          ...
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  PKFACTORY_LOG_LEVEL: "Warn",
                  PKFACTORY_MODE: "desktop",
                  PKFACTORY_PORT: "4001",
                  PKFACTORY_HOST: "0.0.0.0",
                  PKFACTORY_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  PKFACTORY_DEV_ALLOWED_ORIGINS:
                    "https://host.example.ts.net, https://phone.example.ts.net ",
                  PKFACTORY_NO_BROWSER: "true",
                  PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  PKFACTORY_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pkfactory-cli-config-flags-base");
      const derivedPaths = yield* deriveExplicitServerPaths(
        baseDir,
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  PKFACTORY_PORT: "4001",
                  PKFACTORY_HOST: "0.0.0.0",
                  PKFACTORY_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  PKFACTORY_NO_BROWSER: "false",
                  PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  PKFACTORY_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pkfactory-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  PKFACTORY_BOOTSTRAP_FD: String(fd),
                  PKFACTORY_NO_BROWSER: "true",
                  PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  PKFACTORY_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/pkfactory-bootstrap-home";
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
          pkfactoryHome: baseDir,
          noBrowser: true,
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pkfactory-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-cli-config-settings-",
      });
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "pkfactory-cli-config-headless-base");
      const derivedPaths = yield* deriveExplicitServerPaths(baseDir, undefined);
