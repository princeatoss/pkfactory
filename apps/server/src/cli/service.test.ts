...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/pkfactory.service",
  logPath: "/home/me/.pkfactory/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "PK Factory service",
      "  Status: installed · pkfactory@0.0.29",
      "  Unit: /home/me/.config/systemd/user/pkfactory.service",
      "  Logs: /home/me/.pkfactory/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx pkfactory@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx pkfactory@0.0.32-nightly.1 service update");
  expect(output).not.toContain("pkfactory@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx pkfactory@0.0.29 service update");
  expect(output).not.toContain("npx pkfactory@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "pkfactory@0.0.32-nightly.1 (newer than this pkfactory@0.0.31 CLI)");
  assert.include(output, "npx pkfactory@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx pkfactory@latest service update");
});
...
it.layer(Layer.mergeAll(NodeServices.layer, NetService.layer))("service commands", (it) => {
    ...
    (command) =>
      Effect.gen(function* () {
        ...
        const baseDir = yield* fs.makeTempDirectoryScoped({
          prefix: "pkfactory-service-cli-test-",
        });
  ...
  it.effect.each(["install", "update"] as const)("%s allows an explicit downgrade", (command) =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-service-cli-test-" });
      const { service, installOptions } = makeTestService(newerServiceStatus);
      vi.spyOn(BootService, "layer").mockReturnValue(
        Layer.succeed(BootService.BootService, service),
      );
