...
/**
 * `pkfactory theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `pkfactory theme set`.
 *
...
import { fromJsonStringPretty, fromLenientJson } from "@pkfactory/shared/schemaJson";
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@pkfactory/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedErrorClass<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from PK Factory, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedErrorClass<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `pkfactory theme clear` to remove the theme.";
  }
}

const envPKFactoryHome = Config.string("PKFACTORY_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then PKFACTORY_HOME,
  // then the default home. A provisioning script exporting PKFACTORY_HOME must
  // not have this one command silently target the default install.
  const envHome = Option.filter(yield* envPKFactoryHome, (value) => value.trim().length > 0);
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `pkfactory theme set ocean` publish ./ocean whenever the cwd happens to
