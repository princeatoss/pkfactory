...
import { WS_METHODS } from "@pkfactory/contracts";
...
describe("RpcInstrumentation", () => {
  it.effect("records success metrics for unary RPC handlers", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "pkfactory_rpc_requests_total", {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "pkfactory_rpc_request_duration", {
          method: "rpc.instrumentation.success",
        }),
        true,
      );
  ...
  it.effect("records direct stream durations from nanosecond clock readings", () =>
    Effect.gen(function* () {
      ...
      const snapshots = yield* Metric.snapshot;
      const snapshot = findHistogramSnapshot(snapshots, "pkfactory_rpc_request_duration", {
