...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@pkfactory/shared/git";
import { compactTraceAttributes } from "@pkfactory/shared/observability";
import { decodeJsonResult } from "@pkfactory/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `pkfactory-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/pkfactory/pre-refresh", headCommit],
