...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@pkfactory/contracts";
import { fromLenientJson } from "@pkfactory/shared/schemaJson";
...
>()("pkfactory/vcs/VcsProjectConfig") {}
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".pkfactory", "vcs.json");
