...
import { GitCommandError, type ReviewDiffFileContentsInput } from "@pkfactory/contracts";
import { ServerConfig } from "../config.ts";
import { makeGitVcsDriverCore, splitNullSeparatedGitStdoutPaths } from "./GitVcsDriverCore.ts";
import * as GitVcsDriver from "./GitVcsDriver.ts";

const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "pkfactory-git-vcs-driver-test-",
});
    ...
    it.effect("makes background upstream status fetches non-interactive", () =>
      Effect.gen(function* () {
        ...
        const envKeys = [
          ...
          "SSH_ASKPASS",
          "SSH_ASKPASS_REQUIRE",
          "PKFACTORY_TEST_SSH_ASKPASS_LOG",
        ] as const;
        ...
        yield* fileSystem.writeFileString(
          ...
          [
            "#!/bin/sh",
            'printf "GCM_INTERACTIVE=%s\\n" "${GCM_INTERACTIVE:-}" > "$PKFACTORY_TEST_SSH_ASKPASS_LOG"',
            'printf "GIT_ASKPASS=%s\\n" "${GIT_ASKPASS:-}" >> "$PKFACTORY_TEST_SSH_ASKPASS_LOG"',
            'printf "GIT_TERMINAL_PROMPT=%s\\n" "${GIT_TERMINAL_PROMPT:-}" >> "$PKFACTORY_TEST_SSH_ASKPASS_LOG"',
            'printf "SSH_ASKPASS=%s\\n" "${SSH_ASKPASS:-}" >> "$PKFACTORY_TEST_SSH_ASKPASS_LOG"',
            'printf "SSH_ASKPASS_REQUIRE=%s\\n" "${SSH_ASKPASS_REQUIRE:-}" >> "$PKFACTORY_TEST_SSH_ASKPASS_LOG"',
            "exit 1",
            "",
          ].join("\n"),
        );
        ...
        yield* Effect.gen(function* () {
          ...
          process.env.PKFACTORY_TEST_SSH_ASKPASS_LOG = sshLogPath;
    ...
    it.effect("ensureRemote reuses an existing remote across ssh/https transport variants", () =>
      Effect.gen(function* () {
        ...
        const reusedForSsh = yield* driver.ensureRemote({
          ...
          url: "git@github.com:princeatoss/pkfactory.git",
        });
        ...
        const reusedForSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com/princeatoss/pkfactory",
        });
        ...
        const reusedForBareSshScheme = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://github.com/princeatoss/pkfactory",
        });
        ...
        const reusedForSshPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com:22/princeatoss/pkfactory",
        });
        ...
        const reusedForSshWithPort = yield* driver.ensureRemote({
          cwd,
          preferredName: "pingdotgg",
          url: "ssh://git@github.com:22/princeatoss/pkfactory.git",
        });
        ...
        const addedForFork = yield* driver.ensureRemote({
          ...
          url: "git@github.com:octocat/pkfactory.git",
        });
    ...
    it.effect("creates a worktree from the latest fetched remote commit", () =>
      Effect.gen(function* () {
        ...
        yield* driver.createWorktree({
          ...
          newRefName: "pkfactory/fetched-origin",
        ...
        assert.equal(
          yield* driver.readConfigValue(
            worktreePath,
            "branch.pkfactory/fetched-origin.gh-merge-base",
          ),
          initialBranch,
        );
        assert.equal(
          yield* driver.readConfigValue(worktreePath, "branch.pkfactory/fetched-origin.remote"),
          null,
        );
