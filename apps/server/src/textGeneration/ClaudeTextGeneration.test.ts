...
import { ClaudeSettings, ProviderInstanceId } from "@pkfactory/contracts";
import { isHostWindows } from "@pkfactory/shared/hostProcess";
...
const ClaudeTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "pkfactory-claude-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "}",
        "",
        "const argsMustContain = process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_CONTAIN;",
        "if (argsMustContain && !args.includes(argsMustContain)) {",
        '  fail("args missing expected content", 2);',
        "}",
        "",
        "const argsMustNotContain = process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;",
        "if (argsMustNotContain && args.includes(argsMustNotContain)) {",
        '  fail("args contained forbidden content", 3);',
        "}",
        "",
        "const stdinMustContain = process.env.PKFACTORY_FAKE_CLAUDE_STDIN_MUST_CONTAIN;",
        "if (stdinMustContain && !stdinContent.includes(stdinMustContain)) {",
        '  fail("stdin missing expected content", 4);',
        "}",
        "",
        "const configDirMustBe = process.env.PKFACTORY_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;",
        ...
        "}",
        "",
        "const stderrText = process.env.PKFACTORY_FAKE_CLAUDE_STDERR;",
        "if (stderrText) {",
        '  process.stderr.write(stderrText + "\\n");',
        "}",
        "",
        'process.stdout.write(process.env.PKFACTORY_FAKE_CLAUDE_OUTPUT ?? "");',
        "process.exitCode = Number(process.env.PKFACTORY_FAKE_CLAUDE_EXIT_CODE ?? 0);",
        "",
      ].join("\n"),
    );
...
function withFakeClaudeEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-claude-text-" });
    const binDir = yield* makeFakeClaudeBinary(tempDir);
    const pathDelimiter = (yield* isHostWindows) ? ";" : ":";
    const previousPath = process.env.PATH;
    const previousOutput = process.env.PKFACTORY_FAKE_CLAUDE_OUTPUT;
    const previousExitCode = process.env.PKFACTORY_FAKE_CLAUDE_EXIT_CODE;
    const previousStderr = process.env.PKFACTORY_FAKE_CLAUDE_STDERR;
    const previousArgsMustContain = process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_CONTAIN;
    const previousArgsMustNotContain = process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;
    const previousStdinMustContain = process.env.PKFACTORY_FAKE_CLAUDE_STDIN_MUST_CONTAIN;
    const previousConfigDirMustBe = process.env.PKFACTORY_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;

    yield* Effect.acquireRelease(
      Effect.sync(() => {
        process.env.PATH = `${binDir}${pathDelimiter}${previousPath ?? ""}`;
        process.env.PKFACTORY_FAKE_CLAUDE_OUTPUT = input.output;

        if (input.exitCode !== undefined) {
          process.env.PKFACTORY_FAKE_CLAUDE_EXIT_CODE = String(input.exitCode);
        } else {
          delete process.env.PKFACTORY_FAKE_CLAUDE_EXIT_CODE;
        }

        if (input.stderr !== undefined) {
          process.env.PKFACTORY_FAKE_CLAUDE_STDERR = input.stderr;
        } else {
          delete process.env.PKFACTORY_FAKE_CLAUDE_STDERR;
        }

        if (input.argsMustContain !== undefined) {
          process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_CONTAIN = input.argsMustContain;
        } else {
          delete process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_CONTAIN;
        }

        if (input.argsMustNotContain !== undefined) {
          process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN = input.argsMustNotContain;
        } else {
          delete process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;
        }

        if (input.stdinMustContain !== undefined) {
          process.env.PKFACTORY_FAKE_CLAUDE_STDIN_MUST_CONTAIN = input.stdinMustContain;
        } else {
          delete process.env.PKFACTORY_FAKE_CLAUDE_STDIN_MUST_CONTAIN;
        }

        if (input.configDirMustBe !== undefined) {
          process.env.PKFACTORY_FAKE_CLAUDE_CONFIG_DIR_MUST_BE = input.configDirMustBe;
        } else {
          delete process.env.PKFACTORY_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
        }
      }),
      () =>
        Effect.sync(() => {
          ...
          if (previousOutput === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_OUTPUT;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_OUTPUT = previousOutput;
          }

          if (previousExitCode === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_EXIT_CODE;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_EXIT_CODE = previousExitCode;
          }

          if (previousStderr === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_STDERR;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_STDERR = previousStderr;
          }

          if (previousArgsMustContain === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_CONTAIN;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_CONTAIN = previousArgsMustContain;
          }

          if (previousArgsMustNotContain === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_ARGS_MUST_NOT_CONTAIN = previousArgsMustNotContain;
          }

          if (previousStdinMustContain === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_STDIN_MUST_CONTAIN;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_STDIN_MUST_CONTAIN = previousStdinMustContain;
          }

          if (previousConfigDirMustBe === undefined) {
            delete process.env.PKFACTORY_FAKE_CLAUDE_CONFIG_DIR_MUST_BE;
          } else {
            process.env.PKFACTORY_FAKE_CLAUDE_CONFIG_DIR_MUST_BE = previousConfigDirMustBe;
          }
        }),
    );
