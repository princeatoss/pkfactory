...
import { GrokSettings, ProviderInstanceId } from "@pkfactory/contracts";
...
const GrokTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "pkfactory-grok-text-generation-test-",
...
function withFakeAcpGrok<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "pkfactory-grok-text-acp-"));
...
it.layer(GrokTextGenerationTestLayer)("GrokTextGeneration", (it) => {
  it.effect("uses ACP with disabled tool capabilities and forwards the requested model id", () => {
    const requestLogDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "pkfactory-grok-text-log-"),
    );
    ...
    return withFakeAcpGrok(
      {
        ...
        PKFACTORY_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("extracts the JSON object when Grok wraps it in conversational text", () =>
    withFakeAcpGrok(
      {
        PKFACTORY_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect("surfaces ACP request failures as text generation errors", () =>
    withFakeAcpGrok(
      {
        PKFACTORY_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({ branch: "unreachable" }),
      },
      (textGeneration) =>
        Effect.gen(function* () {
  ...
  it.effect("fails with TextGenerationError when output is unparseable JSON", () =>
    withFakeAcpGrok(
      {
        PKFACTORY_ACP_PROMPT_RESPONSE_TEXT: "totally not json output from a confused model",
      },
      (textGeneration) =>
        Effect.gen(function* () {
          const error = yield* Effect.flip(
