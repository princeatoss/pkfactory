...
> {
  return Effect.gen(function* () {
    const remoteDir = yield* makeTempDir("pkfactory-git-remote-");
...
function makeManager(input?: {
  ...
  const serverConfigLayer = ServerConfig.layerTest(process.cwd(), {
    prefix: "pkfactory-git-manager-test-",
  });
...
const GitManagerTestLayer = GitVcsDriver.layer.pipe(
  Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "pkfactory-git-manager-test-" })),
  Layer.provideMerge(VcsProcess.layer),
  Layer.provideMerge(NodeServices.layer),
);
...
it.layer(GitManagerTestLayer)("GitManager", (it) => {
  it.effect("status includes draft PR metadata when branch already has a draft PR", () =>
    Effect.gen(function* () {
      const repoDir = yield* makeTempDir("pkfactory-git-manager-");
  ...
  it.effect("status returns an explicit non-repo result for non-git directories", () =>
    Effect.gen(function* () {
      const cwd = yield* makeTempDir("pkfactory-git-manager-non-repo-");
      const { manager } = yield* makeManager();
  ...
  it.effect("status returns an explicit non-repo result for deleted directories", () =>
    Effect.gen(function* () {
      const rootDir = yield* makeTempDir("pkfactory-git-manager-missing-dir-");
            ...
            JSON.stringify([
              {
                ...
                url: "https://github.com/princeatoss/pkfactory/pull/216",
                baseRefName: "main",
            ...
            JSON.stringify([
              {
                ...
                url: "https://github.com/princeatoss/pkfactory/pull/217",
                baseRefName: "main",
            ...
            JSON.stringify([
              {
                ...
                url: "https://github.com/princeatoss/pkfactory/pull/215",
                baseRefName: "main",
            ...
            JSON.stringify([
              {
                ...
                url: "https://github.com/princeatoss/pkfactory/pull/214",
                baseRefName: "main",
              ...
              JSON.stringify([
                {
                  ...
                  url: "https://github.com/princeatoss/pkfactory/pull/1661",
                  baseRefName: "main",
                  headRefName: "main",
                  state: "OPEN",
                  updatedAt: "2026-04-01T15:00:00Z",
                  isCrossRepository: true,
                  headRepository: {
                    nameWithOwner: "lnieuwenhuis/pkfactory",
                  },
                  headRepositoryOwner: {
                    login: "lnieuwenhuis",
                  },
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "pkfactory/pr-488/statemachine"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("pkfactory/pr-488/statemachine");
        expect(status.pr).toEqual({
          number: 488,
          title: "Rebase this PR on latest main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/488",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["checkout", "-b", "pkfactory/pr-777/main"]);
        ...
        const status = yield* manager.status({ cwd: repoDir });
        expect(status.refName).toBe("pkfactory/pr-777/main");
        expect(status.pr).toEqual({
          number: 777,
          title: "Fork PR from main",
          url: "https://github.com/pingdotgg/codething-mvp/pull/777",
              ...
              "effect-atom": JSON.stringify([
                {
                  number: 1618,
                  title: "Correct PR",
                  url: "https://github.com/princeatoss/pkfactory/pull/1618",
                  baseRefName: "main",
              ...
              "upstream/effect-atom": JSON.stringify([
                {
                  number: 1518,
                  title: "Wrong PR",
                  url: "https://github.com/princeatoss/pkfactory/pull/1518",
                  baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* runGit(repoDir, ["remote", "add", "fork-seed", forkDir]);
        yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
        yield* runGit(repoDir, ["checkout", "-b", "pkfactory/pr-142/statemachine"]);
        yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
        yield* configureVisibleRemoteUrlWithLocalRewrite(
          repoDir,
          "fork-seed",
        ...
        const { manager, ghCalls } = yield* makeManager({
          ghScenario: {
            prListByHeadSelector: {
              // @effect-diagnostics-next-line preferSchemaOverJson:off
              "pkfactory/pr-142/statemachine": JSON.stringify([]),
              // @effect-diagnostics-next-line preferSchemaOverJson:off
  ...
  it.effect("accepts fork PR metadata when origin is the fork checkout remote", () =>
    Effect.sync(() => {
      const headContext = {
        headBranch: "pkfactory/git-audit-stability",
        headRepositoryNameWithOwner: "justsomelegs/pkfactory",
        ...
        GitManager.matchesBranchHeadContext(
          {
            ...
            url: "https://github.com/princeatoss/pkfactory/pull/2284",
            baseRefName: "main",
            headRefName: "pkfactory/git-audit-stability",
            state: "open",
            updatedAt: Option.none(),
            isCrossRepository: true,
            headRepositoryNameWithOwner: "justsomelegs/pkfactory",
  ...
  it.effect("generates PR content against the remote base when the local base is stale", () =>
    Effect.gen(function* () {
      ...
      const peerDir = yield* makeTempDir("pkfactory-git-peer-");
  ...
  it.effect("creates cross-repo PRs with the fork owner selector and default base branch", () =>
    Effect.gen(function* () {
      ...
      yield* runGit(repoDir, ["commit", "-m", "Feature commit"]);
      yield* runGit(repoDir, ["push", "-u", "fork-seed", "statemachine"]);
      yield* runGit(repoDir, ["checkout", "-b", "pkfactory/pr-91/statemachine"]);
      yield* runGit(repoDir, ["branch", "--set-upstream-to", "fork-seed/statemachine"]);
      yield* configureVisibleRemoteUrlWithLocalRewrite(
        repoDir,
        "fork-seed",
  ...
  it.effect("preserves both branch materialization failures when the fallback also fails", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        localBranch: "pkfactory/pr-93/feature/missing-fork-branch",
      });
      ...
      const { manager } = yield* makeManager({
        ghScenario: {
          pullRequest: {
            ...
            url: "https://github.com/princeatoss/pkfactory/pull/642",
            baseRefName: "main",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("pkfactory/pr-91/main");
        expect(result.worktreePath).not.toBeNull();
        expect((yield* runGit(repoDir, ["branch", "--show-current"])).stdout.trim()).toBe("main");
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(result.branch).toBe("pkfactory/pr-92/main");
        expect((yield* runGit(repoDir, ["rev-parse", "main"])).stdout.trim()).toBe(localMainBefore);
        expect(
          (yield* runGit(result.worktreePath as string, [
            "rev-parse",
