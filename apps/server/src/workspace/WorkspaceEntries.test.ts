...
const TestLayer = Layer.empty.pipe(
  ...
  Layer.provide(
    ServerConfig.ServerConfig.layerTest(process.cwd(), {
      prefix: "pkfactory-workspace-entries-test-",
    }),
  ),
...
const makeTempDir = Effect.fn(function* (opts?: { prefix?: string; git?: boolean }) {
  ...
  const dir = yield* fileSystem.makeTempDirectoryScoped({
    prefix: opts?.prefix ?? "pkfactory-workspace-entries-",
  });
  if (opts?.git) {
    yield* git(dir, ["init"]);
  }
...
it.layer(TestLayer, { excludeTestServices: true })("WorkspaceEntries", (it) => {
  ...
  describe("search", () => {
    ...
    it.effect("filters and ranks entries by query", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-query-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("supports fuzzy subsequence queries for composer path search", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-fuzzy-query-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
        yield* writeTextFile(cwd, "src/components/composePrompt.ts");
        yield* writeTextFile(cwd, "docs/composition.md");
    ...
    it.effect("prioritizes exact basename matches ahead of broader path matches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-exact-ranking-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("tracks truncation without sorting every fuzzy match", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-fuzzy-limit-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
        yield* writeTextFile(cwd, "src/components/composePrompt.ts");
        yield* writeTextFile(cwd, "docs/composition.md");
    ...
    it.effect("applies the file filter before limiting search results", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-file-limit-" });
        yield* writeTextFile(cwd, "src/index.ts");
    ...
    it.effect("answers an empty file-filtered query with a bounded file listing", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-empty-query-" });
        yield* writeTextFile(cwd, "src/index.ts");
        yield* writeTextFile(cwd, "README.md");
    ...
    it.effect("returns only directories for the directory filter", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-directory-filter-" });
        yield* writeTextFile(cwd, "src/index.ts");
    ...
    it.effect("excludes gitignored paths for git repositories", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-gitignore-", git: true });
    ...
    it.effect("excludes tracked paths that match ignore rules", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: "pkfactory-workspace-tracked-gitignore-",
          git: true,
        });
        yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
        yield* writeTextFile(cwd, "src/keep.ts", "export {};");
    ...
    it.effect("excludes .convex in non-git workspaces", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-non-git-convex-" });
        yield* writeTextFile(cwd, ".convex/local-storage/data.json", "{}");
        yield* writeTextFile(cwd, "src/keep.ts", "export {};");
    ...
    it.effect("supports typo-resistant file search through fff", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-fff-typo-" });
        yield* writeTextFile(cwd, "src/components/Composer.tsx");
    ...
    it.effect("rebuilds the cached index after refresh fails", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-refresh-failure-" });
  ...
  describe("searchContents", () => {
    it.effect("returns content matches with file paths, line numbers, and ranges", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-search-" });
        yield* writeTextFile(
          cwd,
          "src/shapes.ts",
    ...
    it.effect("honors case sensitivity and gitignore rules", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: "pkfactory-workspace-content-ignore-",
          git: true,
        });
    ...
    it.effect("filters whole-word matches by word boundaries without widening ranges", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-whole-word-" });
    ...
    it.effect("finds later whole-word matches in a file after rejected raw matches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-late-whole-word-" });
    ...
    it.effect("treats astral-plane letters as whole word characters", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-astral-word-" });
    ...
    it.effect("matches punctuation-edged whole-word queries including adjacent occurrences", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-punctuation-" });
    ...
    it.effect("matches punctuation-edged regex queries as whole words", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({
          prefix: "pkfactory-workspace-content-regex-punctuation-",
        });
    ...
    it.effect("caps matches per file so one dense file cannot fill the page", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-per-file-cap-" });
    ...
    it.effect("preserves regex escapes during case-insensitive searches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-regex-" });
    ...
    it.effect("preserves invalid regex errors during case-insensitive searches", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-invalid-regex-" });
    ...
    it.effect("maps multi-byte lines to string-indexed ranges", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-content-multibyte-" });
  ...
  describe("browse", () => {
    it.effect("returns matching directories and excludes files", () =>
      Effect.gen(function* () {
        const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
        const path = yield* Path.Path;
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-browse-prefix-" });
    ...
    it.effect("shows dot directories in directory mode and hidden-prefix mode", () =>
      Effect.gen(function* () {
        const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
        const path = yield* Path.Path;
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-browse-hidden-" });
    ...
    it.effect("supports relative paths when cwd is provided", () =>
      Effect.gen(function* () {
        const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
        const path = yield* Path.Path;
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-browse-relative-" });
    ...
    it.effect("returns an empty listing when the OS denies directory access", () =>
      Effect.gen(function* () {
        const workspaceEntries = yield* WorkspaceEntries.WorkspaceEntries;
        const cwd = yield* makeTempDir({ prefix: "pkfactory-workspace-browse-eacces-" });
