...
describe("ResourceMonitorBinary", () => {
  ...
  it.effect("skips Linux libc detection on Windows", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "pkfactory-resource-monitor-binary-",
      });
      const binaryPath = `${baseDir}/pkfactory-resource-monitor.exe`;
      ...
      const service = yield* ResourceMonitorBinary.make().pipe(
        ...
        Effect.provideService(HostProcessEnvironment, {
          PKFACTORY_RESOURCE_MONITOR_PATH: binaryPath,
        }),
      );
  ...
  it.effect("resolves an executable override", () =>
    Effect.gen(function* () {
      ...
      const binaryPath = `${baseDir}/pkfactory-resource-monitor`;
      yield* fileSystem.writeFileString(binaryPath, "binary");
