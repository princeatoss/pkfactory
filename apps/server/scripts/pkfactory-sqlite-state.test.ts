...
import { runSqliteState } from "./pkfactory-sqlite-state.ts";
...
it.layer(NodeServices.layer)("pkfactory-sqlite-state", (it) => {
  it.effect("reports each invalid SQL source with a specific error", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-sqlite-state-input-",
      });
  ...
  it.effect("queries an isolated database through Effect SQL", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-sqlite-state-query-",
      });
  ...
  it.effect("backs up isolated state before writes and refuses the shared home", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-sqlite-state-exec-" });
      yield* createFixtureDatabase(baseDir);
      ...
      const aliasParent = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-sqlite-state-alias-",
      });
