...
import { fromJsonStringPretty } from "@pkfactory/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start PK Factory once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.pkfactory database. Use an isolated --base-dir.";
  }
}
...
export const pkfactorySqliteStateCommand = Command.make(
  "pkfactory-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit PK Factory base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated PK Factory SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(pkfactorySqliteStateCommand, { version: "0.0.0" }).pipe(
