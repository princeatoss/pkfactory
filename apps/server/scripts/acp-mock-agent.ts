...
const requestLogPath = process.env.PKFACTORY_ACP_REQUEST_LOG_PATH;
const exitLogPath = process.env.PKFACTORY_ACP_EXIT_LOG_PATH;
const antigravityProfile = process.env.PKFACTORY_ACP_ANTIGRAVITY === "1";
const emitToolCalls = process.env.PKFACTORY_ACP_EMIT_TOOL_CALLS === "1";
const emitInterleavedAssistantToolCalls =
  process.env.PKFACTORY_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS === "1";
const emitGenericToolPlaceholders =
  process.env.PKFACTORY_ACP_EMIT_GENERIC_TOOL_PLACEHOLDERS === "1";
const emitAskQuestion = process.env.PKFACTORY_ACP_EMIT_ASK_QUESTION === "1";
const emitXAiAskUserQuestion = process.env.PKFACTORY_ACP_EMIT_XAI_ASK_USER_QUESTION === "1";
const emitXAiExitPlanMode = process.env.PKFACTORY_ACP_EMIT_XAI_EXIT_PLAN_MODE === "1";
const emitXAiPlanMdWrite = process.env.PKFACTORY_ACP_EMIT_XAI_PLAN_MD_WRITE === "1";
const emitXAiPromptCompleteThenHang =
  process.env.PKFACTORY_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG === "1";
const emitXAiRateLimitThenHang = process.env.PKFACTORY_ACP_EMIT_XAI_RATE_LIMIT_THEN_HANG === "1";
const emitXAiAskUserQuestionThenHang =
  process.env.PKFACTORY_ACP_EMIT_XAI_ASK_USER_QUESTION_THEN_HANG === "1";
const emitContentThenHang = process.env.PKFACTORY_ACP_EMIT_CONTENT_THEN_HANG === "1";
const emitPlanThenHang = process.env.PKFACTORY_ACP_EMIT_PLAN_THEN_HANG === "1";
const emitActiveToolThenHang = process.env.PKFACTORY_ACP_EMIT_ACTIVE_TOOL_THEN_HANG === "1";
const emitForeignSessionUpdates = process.env.PKFACTORY_ACP_EMIT_FOREIGN_SESSION_UPDATES === "1";
const waitForResumeRelease = process.env.PKFACTORY_ACP_WAIT_FOR_RESUME_RELEASE === "1";
const completeFirstPromptOnCancel =
  process.env.PKFACTORY_ACP_COMPLETE_FIRST_PROMPT_ON_CANCEL === "1";
const floodStderr = process.env.PKFACTORY_ACP_FLOOD_STDERR === "1";
const hangPromptForever = process.env.PKFACTORY_ACP_HANG_PROMPT_FOREVER === "1";
const hangFirstPromptForever = process.env.PKFACTORY_ACP_HANG_FIRST_PROMPT_FOREVER === "1";
const emitLateUpdateAfterCancel = process.env.PKFACTORY_ACP_EMIT_LATE_UPDATE_AFTER_CANCEL === "1";
const omitXAiPromptCompleteStopReason =
  process.env.PKFACTORY_ACP_OMIT_XAI_PROMPT_COMPLETE_STOP_REASON === "1";
const failLoadSession = process.env.PKFACTORY_ACP_FAIL_LOAD_SESSION === "1";
const emitLoadReplay = process.env.PKFACTORY_ACP_EMIT_LOAD_REPLAY === "1";
const hangLoadSessionAfterReplay = process.env.PKFACTORY_ACP_HANG_LOAD_SESSION_AFTER_REPLAY === "1";
const delayLoadSessionAfterReplay =
  process.env.PKFACTORY_ACP_DELAY_LOAD_SESSION_AFTER_REPLAY === "1";
const loadSessionDelayMs = Number(process.env.PKFACTORY_ACP_LOAD_SESSION_DELAY_MS ?? "5000");
const emitStaleXAiPromptCompleteBeforeSecondHang =
  process.env.PKFACTORY_ACP_EMIT_STALE_XAI_PROMPT_COMPLETE_BEFORE_SECOND_HANG === "1";
const emitOverlappingXAiPromptCompleteOutOfOrder =
  process.env.PKFACTORY_ACP_EMIT_OVERLAPPING_XAI_PROMPT_COMPLETE_OUT_OF_ORDER === "1";
const failPrompt = process.env.PKFACTORY_ACP_FAIL_PROMPT === "1";
const failSetConfigOption = process.env.PKFACTORY_ACP_FAIL_SET_CONFIG_OPTION === "1";
const exitOnSetConfigOption = process.env.PKFACTORY_ACP_EXIT_ON_SET_CONFIG_OPTION === "1";
const promptResponseText = process.env.PKFACTORY_ACP_PROMPT_RESPONSE_TEXT;
const initialGrokReasoningEffort =
  process.env.PKFACTORY_ACP_INITIAL_GROK_REASONING_EFFORT?.trim() || undefined;
const promptDelayMs = Number(process.env.PKFACTORY_ACP_PROMPT_DELAY_MS ?? "0");
const permissionOptionIds = {
  allowOnce: process.env.PKFACTORY_ACP_ALLOW_ONCE_OPTION_ID ?? "allow-once",
  allowAlways: process.env.PKFACTORY_ACP_ALLOW_ALWAYS_OPTION_ID ?? "allow-always",
  rejectOnce: process.env.PKFACTORY_ACP_REJECT_ONCE_OPTION_ID ?? "reject-once",
};
const omitAllowAlways = process.env.PKFACTORY_ACP_OMIT_ALLOW_ALWAYS === "1";
const permissionRequestCount = Math.max(
  1,
  Number(process.env.PKFACTORY_ACP_PERMISSION_REQUEST_COUNT ?? "1") || 1,
);
    ...
    Effect.gen(function* () {
      ...
      if (emitToolCalls) {
        ...
        for (let index = 0; index < permissionRequestCount; index++) {
          const command =
            index > 0
              ? (process.env.PKFACTORY_ACP_SECOND_PERMISSION_COMMAND ?? "cat server/package.json")
              : "cat server/package.json";
          const permission = yield* agent.client.requestPermission({
            sessionId: requestedSessionId,
            toolCall: {
              toolCallId: index === 0 ? toolCallId : `${toolCallId}-${index + 1}`,
              title: process.env.PKFACTORY_ACP_PERMISSION_TITLE ?? `\`${command}\``,
              kind: "execute",
              status: "pending",
              rawInput: {
                variant: "Bash",
      ...
      if (emitXAiPlanMdWrite) {
        ...
        const planRoot = process.env.PKFACTORY_ACP_PLAN_ROOT ?? "/tmp/mock-home/.grok";
  ...
  yield* agent.handleUnknownExtRequest((method, params) => {
    if (method === "_test/environment") {
      return Effect.succeed({
        inherited: process.env.PKFACTORY_ACP_RUNTIME_AMBIENT === "sentinel",
        explicit: process.env.PKFACTORY_ACP_RUNTIME_EXPLICIT === "kept",
      });
    }
