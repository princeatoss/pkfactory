...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __PKFACTORY_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __PKFACTORY_BUILD_RELAY_URL__: JSON.stringify(repoEnv.PKFACTORY_RELAY_URL?.trim() ?? ""),
        __PKFACTORY_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.PKFACTORY_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __PKFACTORY_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __PKFACTORY_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __PKFACTORY_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __PKFACTORY_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
