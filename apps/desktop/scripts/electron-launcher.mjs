// This file mostly exists because we want dev mode to say "PK Factory (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "PK Factory (Dev)" : "PK Factory (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.pkfactory.pkfactory.dev.${devBundleIdSuffix || "local"}`
  : "com.pkfactory.pkfactory";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["pkfactory-dev"] : ["pkfactory"];
...
// oxlint-disable-next-line pkfactory/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["PKFACTORY_PORT", environment.PKFACTORY_PORT],
    ["PKFACTORY_HOME", environment.PKFACTORY_HOME],
    ["PKFACTORY_COMMIT_HASH", environment.PKFACTORY_COMMIT_HASH],
    ["PKFACTORY_OTLP_TRACES_URL", environment.PKFACTORY_OTLP_TRACES_URL],
    ["PKFACTORY_OTLP_EXPORT_INTERVAL_MS", environment.PKFACTORY_OTLP_EXPORT_INTERVAL_MS],
    ["PKFACTORY_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --pkfactory-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the PK Factory name.
