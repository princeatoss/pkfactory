...
const remoteDebuggingPort = process.env.PKFACTORY_DESKTOP_REMOTE_DEBUGGING_PORT?.trim();
// oxlint-disable-next-line pkfactory/no-global-process-runtime -- Standalone dev script has no Effect runtime.
...
if (devProtocolClient) {
  childEnv.PKFACTORY_DESKTOP_APP_USER_MODEL_ID = devProtocolClient.appBundleId;
  childEnv.PKFACTORY_DESKTOP_PROTOCOL_REGISTRATION_MANAGED = "1";
}
...
function cleanupStaleDevApps() {
  ...
  NodeChildProcess.spawnSync("pkill", ["-f", "--", `--pkfactory-dev-root=${desktopDir}`], {
    stdio: "ignore",
  });
}
...
function startApp() {
  ...
  const launchArgs = devProtocolClient
    ? electronArgs
    : [...electronArgs, `--pkfactory-dev-root=${desktopDir}`, "dist-electron/main.cjs"];
