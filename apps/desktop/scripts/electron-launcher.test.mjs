...
describe("electron development launcher", () => {
  it("uses captured values only as fallbacks for a live runner environment", () => {
    ...
    assert.include(
      script,
      "exec '/repo/node_modules/electron/Electron' --pkfactory-dev-root='/repo/apps/desktop' '/repo/apps/desktop/dist-electron/main.cjs' \"$@\"",
    );
  });
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/PK Factory (Dev).app",
      "PK Factory (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "PK Factory (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/PK Factory (Dev).app/Contents/MacOS/PK Factory (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/PK Factory (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/PK Factory (Dev).app/Contents/MacOS/Electron'",
    );
