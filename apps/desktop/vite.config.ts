...
const shouldLaunchElectronAfterPack = process.env.PKFACTORY_DESKTOP_DEV === "1";
  ...
  run: {
    tasks: {
      build: {
        ...
        dependsOn: ["pkfactory#build"],
        cache: false,
      },
      dev: {
        command:
          "node scripts/build-browser-secret.mjs && node scripts/build-preview-annotation-css.mjs && cross-env PKFACTORY_DESKTOP_DEV=1 vp pack --watch",
        dependsOn: ["pkfactory#build"],
        cache: false,
      },
      "dev:bundle": {
  ...
  pack: [
    {
      ...
      clean: true,
      deps: {
        alwaysBundle: (id) => id.startsWith("@pkfactory/"),
      },
