...
> =>
  Effect.gen(function* () {
    ...
    const tempDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "pkfactory-wsl-server-tree-test-",
    });
    ...
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        const marker = yield* fileSystem.readFileString(
          path.join(root, "pkfactory-wsl-server-tree.json"),
        );
  ...
  it.effect("sweeps stale version directories and leftover partials after extraction", () =>
    withTempDir((tempDir) =>
      Effect.gen(function* () {
        ...
        // PKFACTORY_HOME is set to tempDir, so the desktop state dir resolves to
        // <tempDir>/userdata (no .pkfactory segment).
