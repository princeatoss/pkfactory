...
const SERVER_ENTRY_SOURCE = 'console.log("pkfactory wsl runtime test server");';
...
describe("buildWslNodeEnvPreamble", () => {
  it("passes the required Node engine range into the shared resolver", () => {
    ...
    expect(preamble).toContain("PKFACTORY_NODE_ENGINE_RANGE='^22.16 || ^23.11 || >=24.10'");
    expect(preamble.indexOf("PKFACTORY_NODE_ENGINE_RANGE=")).toBeLessThan(
  ...
  it("keeps the shared resolver permissive when no Node engine range is provided", () => {
    expect(buildWslNodeEnvPreamble()).toContain("PKFACTORY_NODE_ENGINE_RANGE=''");
  });
});
...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    const script = buildWslRuntimeInstallScript(
      "/mnt/c/Program Files/PK Factory/wsl-runtime.tar.gz",
      "1.2.3-x64",
      "b".repeat(64),
    );

    expect(script).toContain('runtime_parent="$HOME/.pkfactory/wsl-runtime"');
    ...
    expect(script).toContain(
      "tar -xzf '/mnt/c/Program Files/PK Factory/wsl-runtime.tar.gz' -C \"$runtime_tmp\"",
    );
  ...
  it("verifies the archive digest before extracting, and only on a cache miss", () => {
    ...
    expect(script).toContain(
      "archive_sha=$(sha256sum '/mnt/c/Program Files/PK Factory/wsl-runtime.tar.gz' | cut -d ' ' -f 1)",
    );
  ...
  it("treats a runtime whose native payload went missing as a cache miss", () => {
    ...
    expect(script).toContain(
      '    [ -f "${candidate%/*}/pkfactory-wsl-node-pty.json" ] || continue',
    );
  ...
  it("re-hashes the server entry against the digest the install recorded", () => {
    ...
    expect(script).toContain(
      `printf '%s\\n' "$installed_entry_digest" > "$runtime_tmp/.pkfactory-wsl-runtime-ready"`,
    );
    ...
    const markerWritten = script.indexOf('> "$runtime_tmp/.pkfactory-wsl-runtime-ready"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.pkfactory/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.pkfactory/wsl-runtime/1.2.3-x64",
    );
  ...
  it("prunes completed runtimes except the current and newest previous cache", () => {
    ...
    expect(script).toContain('[ -f "$candidate/.pkfactory-wsl-runtime-ready" ] || continue');
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain(
      'rm -f "$HOME/.pkfactory/wsl-runtime/1.2.3_x64/.pkfactory-wsl-runtime-ready"',
    );
  ...
  const createFixture = () => {
    const result = runShell(
      [
        ...
        `printf '%s' '{"arch":"x64"}' > "$stage/node_modules/node-pty/prebuilds/linux-x64/pkfactory-wsl-node-pty.json"`,
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.pkfactory/wsl-runtime`,
      runtimeRoot: `${work}/home/.pkfactory/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.pkfactory/wsl-runtime/${runtimeId}/apps/server/dist/bin.mjs`,
      installScript,
  ...
  it("extracts once when two installs race for the same cache", () => {
    ...
    const raced = runShell(
      [
        ...
        'PATH="$work/bin:$PATH"',
        "export PATH",
        `cat > "$work/install.sh" <<'PKFACTORY_INSTALL_SCRIPT'`,
        fixture.installScript(),
        "PKFACTORY_INSTALL_SCRIPT",
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-current/.pkfactory-wsl-runtime-ready"',
        'printf ready > "$runtime_parent/sha256-previous/.pkfactory-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `cat > ${sh(`${fixture.work}/select.sh`)} <<'PKFACTORY_SELECT_SCRIPT'`,
        fixture.installScript(),
        "PKFACTORY_SELECT_SCRIPT",
  ...
  it("prunes a selected cache after its prepare-to-spawn grace period expires", () => {
    ...
    const result = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-previous/.pkfactory-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        `touch -d "10 minutes ago" ${sh(`${fixture.runtimeRoot}/.pkfactory-wsl-runtime-selected`)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `HOME=${sh(`${fixture.work}/home`)}`,
        "export HOME",
        buildWslRuntimePruneScript("sha256-current"),
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        `runtime_parent=${sh(fixture.runtimeParent)}`,
        'rm "$runtime_root/.pkfactory-wsl-runtime-ready"',
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.pkfactory/wsl-runtime"',
        'mkdir -p "$runtime_parent"',
        'make_ready() { mkdir -p "$runtime_parent/$1/apps/server/dist"; printf ready > "$runtime_parent/$1/.pkfactory-wsl-runtime-ready"; }',
