...
vi.mock("electron", async (importOriginal) => ({
  ...
  session: {
    fromPartition: vi.fn(() => ({
      getUserAgent: vi.fn(() => "Mozilla/5.0 Electron/41.5.0 pkfactory/1.2.3"),
...
function makeFakeBrowserWindow() {
  ...
  const webContents = {
    copyImageAt: vi.fn(),
    getURL: vi.fn(() => "pkfactory-dev://app/"),
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.mock(PreviewManager.PreviewManager)({
          ...
          isBrowserPartition: (partition) => partition.startsWith("persist:pkfactory-preview-"),
          getBrowserPartition: () => Effect.succeed("persist:pkfactory-preview-test"),
          reapplyZoom: () =>
            Effect.sync(() => {
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "pkfactory://app/",
        navigationUrl: "pkfactory://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["pkfactory-dev://app/"]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "pkfactory-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "pkfactory-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "pkfactory-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
