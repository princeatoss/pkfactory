...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "PK Factory (Alpha)",
    linuxWmClass: "pkfactory",
    linuxApplicationsDir: "/home/alice/.local/share/applications",
    appImagePath: Option.some("/home/alice/Applications/PK Factory-Code.AppImage"),
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    const entry = DesktopLinuxUrlHandler.renderUrlHandlerDesktopEntry({
      displayName: "PK Factory (Nightly)",
      execTarget: '/home/al ice/Apps/PK Factory "100%" $HOME\\x.AppImage',
      scheme: "pkfactory",
    });

    assert.include(entry, "[Desktop Entry]");
    assert.include(entry, "Name=PK Factory (Nightly)");
    ...
    assert.include(
      entry,
      'Exec="/home/al ice/Apps/PK Factory \\\\"100%%\\\\" \\\\$HOME\\\\\\\\x.AppImage" %U',
    );
    assert.include(entry, "NoDisplay=true");
    assert.notInclude(entry, "StartupWMClass=");
    assert.include(entry, "MimeType=x-scheme-handler/pkfactory;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "pkfactory",
      desktopEntryPath: "/home/alice/.local/share/applications/pkfactory-url-handler.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the pkfactory:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/pkfactory-url-handler.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the pkfactory:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(
        ...
        'Exec="/home/alice/Applications/PK Factory-Code.AppImage" %U',
      );
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/pkfactory;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "pkfactory-url-handler.desktop", "x-scheme-handler/pkfactory"],
        },
      ]);
    });
  });
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/pkfactory-url-handler.desktop",
        }),
      });
