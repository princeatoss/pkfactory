...
import { ConnectionCatalogDocument } from "@pkfactory/client-runtime/platform";
import { EnvironmentId, type PersistedSavedEnvironmentRecord } from "@pkfactory/contracts";
...
function makeLayer(
  ...
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ PKFACTORY_HOME: baseDir })),
    ),
  );
...
const withStore = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "pkfactory-desktop-connection-catalog-test-",
    });
