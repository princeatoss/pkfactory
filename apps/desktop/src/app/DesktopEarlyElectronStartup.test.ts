...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { PKFACTORY_HOME: "/home/user/.pkfactory-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.pkfactory-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        PKFACTORY_HOME: "/home/user/.pkfactory-test",
    ...
    assert.deepEqual(options, {
      linuxWmClass: "pkfactory-dev",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.pkfactory/dev when PKFACTORY_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.pkfactory/dev/desktop-settings.json");
  ...
  it("treats whitespace-only PKFACTORY_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        PKFACTORY_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
