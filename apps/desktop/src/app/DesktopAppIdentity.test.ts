...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("PK Factory"),
          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(
                    input.legacyPathExists === true && path.includes("PK Factory (Alpha)"),
                  ),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"pkfactoryCommitHash":"abcdef1234567890"}'),
          }),
        ),
...
describe("DesktopAppIdentity", () => {
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/PK Factory (Alpha)");
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/PK Factory (Alpha)";
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["PK Factory (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "PK Factory (Alpha)");
