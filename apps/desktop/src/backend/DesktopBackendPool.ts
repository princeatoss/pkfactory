...
//     /.well-known/pkfactory/environment (Windows vs Linux platform).
...
>()("@pkfactory/desktop/backend/DesktopBackendPool") {}
      ...
      function* (failure: DesktopBackendManager.PreflightFailure) {
        ...
        if (!fatal) {
          ...
          yield* electronDialog.showErrorBox(
            ...
            `${reason}\n\nPK Factory will use the Windows backend for this launch and retry WSL the next time the app starts.`,
          );
        ...
        yield* electronDialog.showErrorBox(
          ...
          `${reason}\n\nFalling back to the Windows backend so PK Factory can open. Re-enable the WSL backend from Settings > Connections once the WSL distro is fixed.`,
        );
