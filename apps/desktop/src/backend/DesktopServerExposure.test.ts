...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        DesktopConfig.layerTest({ PKFACTORY_HOME: baseDir, ...env }),
      ),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { PKFACTORY_HOME: input.baseDir, ...input.env };
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "pkfactory-desktop-server-exposure-test-",
    });
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        PKFACTORY_DESKTOP_LAN_HOST: "10.0.0.7",
        PKFACTORY_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
