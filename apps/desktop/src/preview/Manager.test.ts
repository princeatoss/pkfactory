...
import { DESKTOP_PREVIEW_RECORDING_CAPTURE_TRIGGER } from "@pkfactory/contracts";
import type { DesktopPreviewRecordingFrame } from "@pkfactory/contracts";
...
const browserSessionLayer = Layer.succeed(
  ...
  BrowserSession.BrowserSession.of({
    getPartition: () => Effect.succeed("persist:pkfactory-preview-test"),
    isPartition: (partition) => partition.startsWith("persist:pkfactory-preview-"),
...
const environmentLayer = Layer.succeed(
  ...
  DesktopEnvironment.DesktopEnvironment.of({
    browserArtifactsDir: "/tmp/pkfactory/dev/browser-artifacts",
    dirname: "/tmp/pkfactory/desktop",
    path: {
...
describe("PreviewManager", () => {
  ...
  effectIt.effect("captures a PNG screenshot into browser artifacts", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        ...
        expect(capturePage).toHaveBeenCalledOnce();
        expect(mkdir).toHaveBeenCalledWith("/tmp/pkfactory/dev/browser-artifacts");
        ...
        expect(browserWindowConstructor).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            webPreferences: expect.objectContaining({
              preload: "/tmp/pkfactory/desktop/preview-pip-preload.cjs",
    ...
    withManager((manager) =>
      Effect.gen(function* () {
        yield* manager.revealArtifact(
          "/tmp/pkfactory/dev/browser-artifacts/browser-screenshot-test.png",
        );

        expect(showItemInFolder).toHaveBeenCalledWith(
          "/tmp/pkfactory/dev/browser-artifacts/browser-screenshot-test.png",
        );
        const exit = yield* Effect.exit(manager.revealArtifact("/tmp/pkfactory/dev/settings.json"));
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
        expect(error).toMatchObject({
          _tag: "PreviewArtifactPathOutsideDirectoryError",
          artifactPath: "/tmp/pkfactory/dev/settings.json",
          artifactDirectory: "/tmp/pkfactory/dev/browser-artifacts",
        });
  ...
  effectIt.effect("copies screenshot artifacts to the system clipboard", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const artifactPath = "/tmp/pkfactory/dev/browser-artifacts/browser-screenshot-test.png";
        ...
        const exit = yield* Effect.exit(
          manager.copyArtifactToClipboard("/tmp/pkfactory/dev/settings.json"),
        );
        expect(Exit.isFailure(exit)).toBe(true);
        if (Exit.isSuccess(exit)) return;
        const error = Option.getOrThrow(Cause.findErrorOption(exit.cause));
