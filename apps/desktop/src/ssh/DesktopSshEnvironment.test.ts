...
import { SshPasswordPromptError } from "@pkfactory/ssh/errors";
...
function makeTempHomeDir() {
  return Effect.gen(function* () {
    ...
    return yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-ssh-env-test-" });
  });
}
...
describe("sshEnvironment", () => {
  it("keeps prompt presentation diagnostics distinct from the legacy wrapper message", () => {
    ...
    assert.equal(
      ...
      "PK Factory window is not available for SSH authentication.",
    );
  });
