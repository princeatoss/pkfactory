...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "pkfactory-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "pkfactory-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "PKFACTORY_MOBILE_OTLP_TRACES_DATASET=pkfactory-mobile-traces-dev",
        "PKFACTORY_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET=pkfactory-relay-client-traces-dev",
        "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
  ...
  it("replaces stale values while preserving unrelated entries", () => {
    expect(
      reconcileRootEnvPublicConfig(
        [
          "PKFACTORY_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "PKFACTORY_RELAY_URL=https://old.example.test",
          "PKFACTORY_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "PKFACTORY_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "PKFACTORY_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
