...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    PKFACTORY_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    PKFACTORY_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    PKFACTORY_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("PKFACTORY_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("PKFACTORY_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "PKFactoryRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the PK Factory relay through Alchemy."));
