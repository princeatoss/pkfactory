# PK Factory Connect Relay

> [!NOTE]
> Sign in to PK Factory Connect from the app under Settings > Connections.

The relay is the hosted control plane for PK Factory Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal PK Factory traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [PK Factory Connect architecture overview](../../docs/internals/pkfactory-connect-auth-flow.html) for the larger system
design.
...
- Linking PK Factory environments to a cloud account.
...
```sh
vp run --filter pkfactory-relay deploy
```
...
```sh
vp run --filter pkfactory-relay deploy -- --stage prod
vp run --filter pkfactory-relay deploy -- --env-file .env.local
```
...
- [PK Factory Connect Clerk Setup](../../docs/internals/pkfactory-connect.md) for Clerk keys, JWT templates, and sign-up restrictions
  setup.
- [Relay Observability](../../docs/operations/relay-observability.md) for deployment tracing and diagnostics.
- [PK Factory Connect Architecture Overview](../../docs/internals/pkfactory-connect-auth-flow.html) for the full link,
