...
import { RELAY_LINK_PROOF_TYP } from "@pkfactory/shared/relayJwt";
...
const makeRequest = Effect.gen(function* () {
  ...
  const payload = {
    iss: "pkfactory-env:env-link-test",
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.succeed(EnvironmentCredentials.EnvironmentCredentials, {
          create: () => Effect.succeed("pkfactoryenv_credential_secret"),
...
describe("EnvironmentLinker", () => {
  it.effect("uses verified JWT claims when linking an environment", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(result.environmentCredential).toBe("pkfactoryenv_credential_secret");
