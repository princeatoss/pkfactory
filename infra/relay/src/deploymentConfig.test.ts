...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("pkfactory-relay-traces", "prod")).toBe(
      "pkfactory-relay-traces-prod",
    );
    expect(relayResourceNameForStage("pkfactory-relay-traces", "dev_julius")).toBe(
      "pkfactory-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "pkfactoryrelay.com", hash)).toBe(
      "prod-abcdef0123456789.pkfactoryrelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "pkfactoryrelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
