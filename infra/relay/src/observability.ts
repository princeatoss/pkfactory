...
export const RelayObservability = Effect.gen(function* () {
  ...
  const traces = yield* Axiom.Dataset("RelayTracesDataset", {
    name: relayResourceNameForStage("pkfactory-relay-traces", stage),
    kind: "otel:traces:v1",
    description: "PK Factory relay Worker HTTP request spans.",
    retentionDays: 30,
    useRetentionPeriod: true,
  });

  const workerIngestToken = yield* Axiom.ApiToken("RelayWorkerAxiomIngestToken", {
    name: relayResourceNameForStage("pkfactory-relay-otel-ingest", stage),
  ...
  const mobileIngestToken = yield* Axiom.ApiToken("RelayMobileAxiomIngestToken", {
    name: relayResourceNameForStage("pkfactory-mobile-otel-ingest", stage),
    description: "Owned by Alchemy. Scoped OTLP ingest token for PK Factory mobile spans.",
    datasetCapabilities: Output.map(traces.name, (dataset) => ({
      [dataset]: { ingest: ["create" as const] },
    })),
  });
  ...
  const clientIngestToken = yield* Axiom.ApiToken("RelayClientAxiomIngestToken", {
    name: relayResourceNameForStage("pkfactory-relay-client-otel-ingest", stage),
  ...
  yield* Axiom.View("RelayRecentSpansView", {
    name: relayResourceNameForStage("pkfactory-relay-recent-spans", stage),
...
export const makeRelayTraceLayer = (input: {
  ...
  Layer.effect(
    ...
    OtlpTracer.make({
      ...
      resource: {
        serviceName: "pkfactory-relay-worker",
        attributes: {
