...
if [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  pkgname='pkfactory-bin'
elif [[ "$tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+-nightly\.[0-9]{8}\.[0-9]+$ ]]; then
  pkgname='pkfactory-nightly-bin'
else
...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="PK Factory-Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit pkfactory-icon.png
git config user.name 'pkfactory-ci'
git config user.email 'pkfactory-ci@users.noreply.github.com'
git add -A
