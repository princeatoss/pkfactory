...
it("batches codesign calls without changing existing signing options", async () => {
  const options = {
    app: "/tmp/PK Factory.app",
    identity: "Developer ID Application: PK Factory Tools, Inc.",
    keychain: "/tmp/pkfactory.keychain",
    provisioningProfile: "/tmp/pkfactory.provisionprofile",
    optionsForFile: () => ({
      entitlements: "/tmp/pkfactory.entitlements.plist",
