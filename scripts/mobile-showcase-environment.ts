// @effect-diagnostics nodeBuiltinImport:off globalTimers:off globalDate:off - This host-side fixture creates an isolated local PK Factory environment.
...
export const SHOWCASE_PROJECT_ID = "pkfactory";
...
const SHOWCASE_TERMINAL_PROMPT =
  "\u001b[1;32m→\u001b[0m \u001b[1;36mpkfactory\u001b[0m \u001b[1;34mgit:(\u001b[1;31mfeat/remote-command-center\u001b[1;34m)\u001b[0m \u001b[1;33m✗\u001b[0m ";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  ...
  "  \u001b[32m→\u001b[0m  \u001b[2mProject:\u001b[0m  \u001b[1mpkfactory\u001b[0m \u001b[2m— ~/Code/pkfactory\u001b[0m",
  "",
...
const PROJECT_FAVICONS = {
  pkfactory: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    title: "PK Factory",
    directory: "pkfactory",
    repositoryUrl: "https://github.com/princeatoss/pkfactory.git",
    favicon: PROJECT_FAVICONS.pkfactory,
  },
  {
    id: "react",
    title: "React",
...
async function seedPKFactoryWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "pkfactory", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.pkfactory);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the PK Factory mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedPKFactoryWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
