...
const devServerInput = {
  mode: "dev:server",
  pkfactoryHome: "/tmp/pkfactory-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit PKFACTORY_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "PKFACTORY_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "PKFACTORY_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PKFACTORY_HOME, undefined);
        assert.equal(env.PKFACTORY_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PKFACTORY_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("requires the browser flag even when the environment enables auto-open", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { PKFACTORY_NO_BROWSER: "0" },
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: undefined,
          browser: false,
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: "/tmp/custom-pkfactory",
          browser: false,
        ...
        assert.equal(env.PKFACTORY_HOME, path.resolve("/tmp/custom-pkfactory"));
        assert.equal(env.PKFACTORY_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.PKFACTORY_NO_BROWSER, "1");
        assert.equal(env.PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.PKFACTORY_LOG_WS_EVENTS, "1");
        assert.equal(env.PKFACTORY_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            PKFACTORY_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            PKFACTORY_BOOT_SERVICE_UNIT: "pkfactory.service",
          },
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: undefined,
        ...
        assert.equal(env.PKFACTORY_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.PKFACTORY_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            PKFACTORY_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: undefined,
        ...
        assert.equal(env.PKFACTORY_MODE, "web");
        assert.equal(env.PKFACTORY_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PKFACTORY_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom pkfactoryHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          ...
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: "/tmp/my-pkfactory",
          browser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
          host: undefined,
        ...
        assert.equal(env.PKFACTORY_HOME, path.resolve("/tmp/my-pkfactory"));
      }),
    );
      ...
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          baseEnv: {
            ...
            PKFACTORY_MODE: "web",
            PKFACTORY_NO_BROWSER: "0",
            PKFACTORY_HOST: "0.0.0.0",
        ...
        assert.equal(env.PKFACTORY_MODE, undefined);
        assert.equal(env.PKFACTORY_NO_BROWSER, undefined);
        assert.equal(env.PKFACTORY_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.PKFACTORY_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.PKFACTORY_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { PKFACTORY_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.PKFACTORY_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (PKFACTORY_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/PKFACTORY_HOST has to be probed as well.
    ...
    // Only the backend honours --host/PKFACTORY_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { PKFACTORY_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, [
          "PKFACTORY_PORT_OFFSET",
          "PKFACTORY_DEV_INSTANCE",
        ]);
    ...
    // Base-dir precedence (--home-dir > worktree .pkfactory > ambient PKFACTORY_HOME)
    ...
    it.effect("ignores an ambient PKFACTORY_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { PKFACTORY_HOME: "/home/user/.pkfactory" },
          serverOffset: 0,
          webOffset: 0,
          pkfactoryHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit PKFACTORY_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { PKFACTORY_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults PKFACTORY_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.PKFACTORY_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit PKFACTORY_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.PKFACTORY_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves PKFACTORY_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.PKFACTORY_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("pkfactory home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "pkfactory-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({
            ...devServerInput,
            pkfactoryHome: input.pkfactoryHome,
          }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            ...
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { PKFACTORY_HOME: input.ambientHome },
            ),
          );

          return captured?.PKFACTORY_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            pkfactoryHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.pkfactory",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".pkfactory"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .pkfactory over an ambient PKFACTORY_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient PKFACTORY_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            pkfactoryHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.pkfactory"));
        }),
      );
