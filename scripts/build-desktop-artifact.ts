...
import { fromYaml } from "@pkfactory/shared/schemaYaml";
import { HostProcessArchitecture, HostProcessPlatform } from "@pkfactory/shared/hostProcess";
import { clerkFrontendApiHostnameFromPublishableKey } from "@pkfactory/shared/relayAuth";
...
const DESKTOP_APP_ID = "com.pkfactory.pkfactory";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "pkfactory-resource-monitor.exe" : "pkfactory-resource-monitor";
}
...
interface StagePackageJson {
  ...
  readonly pkfactoryCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // PK Factory always passes the user's installed Claude executable to the SDK,
...
export class InvalidAppleTeamIdError extends Schema.TaggedErrorClass<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `PKFACTORY_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedErrorClass<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "PKFACTORY_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedErrorClass<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "PKFACTORY_CLERK_PUBLISHABLE_KEY or PKFACTORY_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedErrorClass<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "PKFACTORY_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
export function resolveMacPasskeySigningConfiguration(
  ...
  const teamId = env.PKFACTORY_APPLE_TEAM_ID?.trim().toUpperCase() ?? "";
  if (!APPLE_TEAM_ID_PATTERN.test(teamId)) {
    throw new InvalidAppleTeamIdError({ teamId });
  }

  const provisioningProfilePath = env.PKFACTORY_MACOS_PROVISIONING_PROFILE?.trim() ?? "";
  if (provisioningProfilePath.length === 0) {
    throw new MissingMacPasskeyProvisioningProfileError();
  }

  const configuredRpDomains = env.PKFACTORY_CLERK_PASSKEY_RP_DOMAINS?.trim();
  let rpDomains: readonly string[];
  if (configuredRpDomains) {
    rpDomains = configuredRpDomains.split(",").map(normalizePasskeyRpDomain);
  } else {
    const publishableKey = env.PKFACTORY_CLERK_PUBLISHABLE_KEY?.trim();
    if (!publishableKey) {
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "PKFACTORY_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("PKFACTORY_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "PKFACTORY_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("PKFACTORY_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("PKFACTORY_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("PKFACTORY_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("PKFACTORY_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("PKFACTORY_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("PKFACTORY_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("PKFACTORY_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("PKFACTORY_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(
    Config.option,
  ),
  ...
  wslPrebuild: Config.string("PKFACTORY_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.boolean(
    "PKFACTORY_DESKTOP_REUSE_RESOURCE_MONITOR",
...
const verifyPackagedBundleIsSelfContained = Effect.fn("verifyPackagedBundleIsSelfContained")(
  function* (input: { readonly asarPath: string; readonly verbose: boolean }) {
    ...
    const probeRoot = yield* fs.makeTempDirectoryScoped({
      prefix: "pkfactory-bundle-selfcheck-",
    });
  ...
  yield* runCommand(
    ChildProcess.make(
      ...
      [
        ...
        "--output",
        path.join(input.stageResourcesDir, "browser-secret", "pkfactory-browser-secret"),
      ],
      { cwd: input.repoRoot },
    ),
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("PKFACTORY_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "PK Factory (Nightly)"
    : (desktopPackageJson.productName ?? "PK Factory");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "PK Factory-Code-${version}-${arch}.${ext}",
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "PK Factory",
          schemes: ["pkfactory", "pkfactory-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "pkfactory",
      icon: "icons",
      category: "Development",
      // electron-builder turns these into MimeType=x-scheme-handler/<scheme>;
      // in the .desktop entry (Exec already gets %U), so browsers can hand
      // pkfactory:// OAuth callbacks to the app.
      protocols: [
        {
          name: "PK Factory",
          schemes: ["pkfactory", "pkfactory-dev"],
...
// node-pty's prebuilds/linux-<arch>/ with a pkfactory marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / PKFACTORY_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(
    path.join(prebuildDir, "pkfactory-wsl-node-pty.json"),
    `${markerJson}\n`,
  );
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "pkfactory-windows-primary-native-probe-",
  });
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  if (!(yield* isFile(resourceMonitorPath))) {
    return yield* new WindowsPackagedPayloadValidationError({
      ...
      packagedAppDir,
      missingFiles: ["resource-monitor/pkfactory-resource-monitor.exe"],
    });
  }
    ...
    const requiredMembers = [
      ...
      ...(wslArch === undefined
        ...
        : [
            ...
            `node_modules/node-pty/prebuilds/linux-${wslArch}/pkfactory-wsl-node-pty.json`,
          ]),
    ];
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `pkfactory-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    pkfactoryCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "PK Factory desktop build",
    author: "PK Factory Tools",
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @pkfactory/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: PKFACTORY_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: PKFACTORY_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription(
      "Build arch, for example arm64/x64/universal (env: PKFACTORY_DESKTOP_ARCH).",
    ),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: PKFACTORY_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: PKFACTORY_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: PKFACTORY_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: PKFACTORY_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: PKFACTORY_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: PKFACTORY_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: PKFACTORY_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription(
      "Mock update server port (env: PKFACTORY_DESKTOP_MOCK_UPDATE_SERVER_PORT).",
    ),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: PKFACTORY_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for PK Factory."),
