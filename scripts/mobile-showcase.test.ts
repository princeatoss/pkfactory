...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(
    ...
    /Unsupported PKFACTORY_SHOWCASE_ANDROID_ABI/u,
  );
});
...
it("expands both appearances into independent upload-ready directories", () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      { appearance: "light", directory: "/captures/apple/iphone-test/light/pkfactory" },
      { appearance: "dark", directory: "/captures/apple/iphone-test/dark/pkfactory" },
    ],
  );
});
...
it("defaults every device to the app's own palette", () => {
  assert.equal(DEFAULT_SHOWCASE_THEME, "pkfactory");
  assert.equal(
