...
import { resolveGitWorktreePath, resolveWorktreePKFactoryHome } from "@pkfactory/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@pkfactory/shared/hostProcess";
...
export const DEFAULT_PKFACTORY_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".pkfactory"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@pkfactory/contracts",
    "--filter=@pkfactory/web",
    "--filter=pkfactory",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=pkfactory", "dev"],
  "dev:web": ["run", "--filter=@pkfactory/web", "dev"],
  "dev:desktop": ["run", "--filter=@pkfactory/desktop", "--filter=@pkfactory/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`PKFACTORY_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("PKFACTORY_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("PKFACTORY_PORT_OFFSET"),
  devInstance: optionalStringConfig("PKFACTORY_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      return Effect.fail(
        new DevRunnerInvalidPortOffsetError({
          configKey: "PKFACTORY_PORT_OFFSET",
    ...
    return Effect.succeed({
      ...
      source: `PKFACTORY_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric PKFACTORY_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed PKFACTORY_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_PKFACTORY_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly pkfactoryHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .pkfactory > ambient PKFACTORY_HOME) is resolved
    // by the caller; an unset pkfactoryHome here genuinely means "use the default".
    const configuredBaseDir = pkfactoryHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.PKFACTORY_HOME = resolvedBaseDir;
    } else {
      delete output.PKFACTORY_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed pkfactory service (an
    // agent working inside PK Factory), these leak through and the child server
    // fails startup with "The service launcher started a different pkfactory version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.PKFACTORY_SERVICE_LAUNCHER_CONTEXT;
    delete output.PKFACTORY_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.PKFACTORY_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.PKFACTORY_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.PKFACTORY_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.PKFACTORY_PORT = String(serverPort);
      ...
      delete output.PKFACTORY_SINGLE_ORIGIN_DEV;
      delete output.PKFACTORY_MODE;
      delete output.PKFACTORY_NO_BROWSER;
      delete output.PKFACTORY_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.PKFACTORY_HOST = host;
    }

    if (!isDesktopMode) {
      output.PKFACTORY_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.PKFACTORY_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.PKFACTORY_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.PKFACTORY_MODE = "web";
      delete output.PKFACTORY_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`PKFACTORY_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["PKFACTORY_PORT_OFFSET", "PKFACTORY_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.pkfactory` — see @pkfactory/shared/devHome for why this must
    // outrank an ambient PKFACTORY_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreePKFactoryHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedPKFactoryHome =
      (input.pkfactoryHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.PKFACTORY_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      pkfactoryHome: resolvedPKFactoryHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.PKFACTORY_HOME ?? (yield* DEFAULT_PKFACTORY_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.PKFACTORY_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          env.PKFACTORY_DEV_ALLOWED_ORIGINS = [
            env.PKFACTORY_DEV_ALLOWED_ORIGINS,
          ...
          // a few chunk requests. Only defaulted, so PKFACTORY_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.PKFACTORY_BUNDLED_DEV === undefined) {
            env.PKFACTORY_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  pkfactoryHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit PK Factory data directory; runtime state is stored under userdata (equivalent to PKFACTORY_HOME). Inside a git worktree this defaults to that worktree's own .pkfactory so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("PKFACTORY_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to PKFACTORY_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("PKFACTORY_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to PKFACTORY_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("PKFACTORY_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to PKFACTORY_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("PKFACTORY_PORT")),
  ),
