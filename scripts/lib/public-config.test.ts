...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.PKFACTORY_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.PKFACTORY_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.PKFACTORY_RELAY_URL).toBeUndefined();
    expect(env.VITE_PKFACTORY_RELAY_URL).toBeUndefined();
    expect(env.PKFACTORY_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.PKFACTORY_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.PKFACTORY_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "PKFACTORY_CLERK_PUBLISHABLE_KEY=pk_root\nPKFACTORY_CLERK_JWT_TEMPLATE=template_root\nPKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nPKFACTORY_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "PKFACTORY_CLERK_PUBLISHABLE_KEY=pk_local\nPKFACTORY_CLERK_JWT_TEMPLATE=template_local\nPKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nPKFACTORY_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).PKFACTORY_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          PKFACTORY_CLERK_PUBLISHABLE_KEY: "pk_ci",
          PKFACTORY_CLERK_JWT_TEMPLATE: "template_ci",
          PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          PKFACTORY_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      PKFACTORY_RELAY_URL: "https://ci.example.test",
      VITE_PKFACTORY_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_PKFACTORY_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          PKFACTORY_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          PKFACTORY_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          PKFACTORY_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          PKFACTORY_RELAY_URL: "https://relay.example.test",
          PKFACTORY_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          PKFACTORY_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          PKFACTORY_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      PKFACTORY_RELAY_URL: "https://relay.example.test",
      VITE_PKFACTORY_RELAY_URL: "https://relay.example.test",
      PKFACTORY_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      PKFACTORY_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      PKFACTORY_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "pkfactory-public-config-"));
