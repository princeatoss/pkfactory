...
const resolveMockUpdateServerConfig = Effect.gen(function* () {
  ...
  const config = yield* Config.all({
    port: Config.port("PKFACTORY_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.withDefault(3000)),
    root: Config.string("PKFACTORY_DESKTOP_MOCK_UPDATE_SERVER_ROOT").pipe(
