...
Messages can contain up to 120,000 characters. If a draft is longer, PK Factory keeps it in the
...
PK Factory from other apps through the system share sheet. Mobile keeps a local copy of each draft
attachment, so you can still preview it and queue messages while offline. Uploads resume when
you reconnect. Drafts and queued messages survive app restarts; signing out of PK Factory Connect keeps
...
PK Factory remembers the last provider, model, and model options you selected and reuses that
...
Model options shown as provider defaults remain display values until you choose them in PK Factory.
PK Factory only sends options you selected explicitly, so an unset reasoning level or service tier can
...
readable and PK Factory shows a warning.
...
On web, desktop, and mobile, select a link to an image or video to open it inside PK Factory.
Workspace image and video links open the file viewer. Links to media outside the workspace
open a media preview.
Videos opened from the file explorer or a file-viewer tab also play inside PK Factory. They
...
PK Factory serves the original file without adding it to attachment storage. If that file is moved
...
runs in an isolated frame with no access to your PK Factory session. On desktop, the integrated
...
project exists there. If it does not, PK Factory selects another environment that has the project.
...
and attachments. PK Factory deletes the local audio file after transcription or cancellation. It sends
...
also reports that skill as a native slash command, PK Factory hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

A skill token runs the skill wherever it sits in your message. PK Factory sends it to each provider in
...
them only there. PK Factory's own commands, such as `/model` and `/plan`, and skills stay available on
any line.

On desktop, press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux from a new thread to
start it in the background. PK Factory opens another new thread and shows an **Open** action for the
