...
PK Factory uses the OpenCode setup on the connected environment. With a remote environment, its
OpenCode login and configuration apply, not the setup on your desktop or phone.

PK Factory requires OpenCode 1.14.19 or newer. It checks the server version before it loads models or
...
Without a server URL, PK Factory starts a local OpenCode server. The process inherits
`OPENCODE_SERVER_PASSWORD` from the environment. A password in the provider settings overrides
that environment value for both the local process and PK Factory.

With a server URL, PK Factory connects to that external server and uses only the password in the
...
PK Factory shows OpenCode's response text and tool results while work runs. The web and desktop apps
also show its task-list progress. A task-list update does not require approval.

If the OpenCode connection closes unexpectedly, PK Factory shows an error. Send another prompt to
reconnect to the same OpenCode session.

## Stop a turn

When you select **Stop**, PK Factory stops the main OpenCode session and all nested child sessions.
PK Factory waits for this cleanup before it marks the turn as stopped or sends the next prompt. It
...
When PK Factory closes an OpenCode session, it also tries to stop the child sessions, but this
teardown is best effort.

## Refresh the model list

PK Factory loads the model list when an enabled OpenCode provider starts and keeps the list in its
...
After changing an OpenCode login or configuration outside PK Factory, open **Settings > Providers**,
select the environment, and choose **Refresh provider status**. Changing the provider's
configuration in PK Factory also replaces that provider connection.
...
PK Factory does not own an external OpenCode server. Native configuration changes on that server can
require its own reload or restart before a refresh returns the new list.

If a refresh fails, PK Factory keeps the last known models, slash commands, and skills. Fix the
...
until the model metadata returns. PK Factory does not switch the thread to the first model in the
list.
