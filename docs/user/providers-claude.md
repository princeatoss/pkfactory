...
This guide is for people who want to use more than one Claude setup in PK Factory. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install PK Factory](./install.md).
...
In PK Factory Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means PK Factory uses Claude Code's normal config directory.

When you set this field, PK Factory points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, PK Factory
...
PK Factory looks for Claude skills in the Claude config directory's `skills` folder and
...
`~/.claude_personal_home/.claude`, which is not where PK Factory looks.

Then add another Claude provider in PK Factory:
...
PK Factory only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so PK Factory keeps separate config directories isolated
...
Add or edit a Claude provider in PK Factory Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. PK Factory stores the value as a server secret and does not
...
PK Factory does not need a special Claude Code Router provider. Treat the router as a Claude
