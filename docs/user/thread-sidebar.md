...
live background work keep the thread active. PK Factory settles from a closed or merged pull request
...
If reordering is unavailable for one environment, update the PK Factory server running in that
...
pill** fallback because their colors are not controlled by PK Factory.

To generate a fresh title from the conversation, open a thread's context menu and choose
**Regenerate title**. While PK Factory is generating it, the action reads **Regenerating…** and cannot
