# Keeping PK Factory in Sync

The PK Factory web or desktop app and the server it connects to work best when they use the same
version. If they do not match, PK Factory shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                                                                                                                                                                                                                             |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the PK Factory Linux background service and for servers run by a current PK Factory desktop app. Select the button and leave PK Factory open while it downloads, installs, restarts, and reconnects. For desktop-app servers this closes and relaunches the desktop app on that machine. If installation fails, the desktop app stays open and reconnects to its server. |
| **Update the desktop app** | Shown for desktop apps that predate remote updates. Open the PK Factory desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                                                                                                                                                                         |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current PK Factory server, and relaunch it with the copied command and any startup options you normally use.                                                                                                                                                                                                         |

The available action depends on how that server was started. PK Factory does not update connected
servers silently in the background.

An older background-service launcher may ask you to run the exact
`npx pkfactory@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `npx pkfactory@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the PK Factory background service, update the service on the host and
pin the same version:

```sh
npx pkfactory@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx pkfactory@latest service update`
...
See [Running PK Factory in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx pkfactory@<client-version>`, replacing
