...
machine running your PK Factory server does that, its themes appear in the theme library alongside
your own, and PK Factory follows them: change the desktop theme and PK Factory retints with it, without
a restart.
...
different theme in Settings afterwards sticks until the next `pkfactory theme set`, and running the same
set again is how you bring clients back. `pkfactory theme clear` removes the setting without changing
what anyone currently has, and `pkfactory theme show` prints the current theme and everything the
machine publishes. Only the environment you are anchored to publishes themes, so a remote client
follows the machine it is connected to, not the device it runs on. PK Factory Mobile keeps its own
...
the standard PK Factory look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the PK Factory state
directory (`~/.pkfactory/userdata/themes/` by default). The filename is the theme id — `nightfall.json`
...
exported from PK Factory (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. PK Factory generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a PK Factory theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of PK Factory does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so PK Factory never reads
