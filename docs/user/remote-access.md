...
Use this when you want to connect to a PK Factory server from another device such as a phone, tablet, or separate desktop app.

## PK Factory Connect troubleshooting

Run `pkfactory connect` on the server machine to authorize it and optionally install the background service.
...
`pkfactory connect status` reports saved authorization and link configuration, not a live reachability
check. If the machine appears offline, run `pkfactory service status` on it and read the displayed log.
...
| Error                                                            | Next step                                                                                                                                                                   |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` / managed tunnel limit reached | Unlink an unused environment in PK Factory Connect, then restart PK Factory on this machine.                                                                                |
| `auth_invalid` / `invalid_bearer`                                | Run `pkfactory connect login`. If the stored credential was revoked, run `pkfactory connect logout`, then `pkfactory connect` and restart the server.                       |
| Expired or invalid link proof                                    | Check the server's date and time, update PK Factory, and restart it. Include the reason and trace ID if it still fails.                                                     |
...
`systemctl --user restart pkfactory.service`; for a foreground server, stop it and run `pkfactory serve` again.
...
`pkfactory pair` finds the running server (the shared `~/.pkfactory` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `pkfactory pair` says so and points you at `npx pkfactory serve` or `npx pkfactory connect`.
...
run a headless server from the CLI, or have the desktop app launch PK Factory over SSH.
...
- HTTPS/WSS-compatible endpoints work from `https://app.pkfactory.codes`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.pkfactory.codes/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
backend with the same server-side behavior as `pkfactory serve --tailscale-serve`, then the server asks
...
For `https://app.pkfactory.codes`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
Run the server with `pkfactory serve`.

```bash
npx pkfactory serve --host "$(tailscale ip -4)"
```

`pkfactory serve` starts the server without opening a browser and prints:
...
Use `pkfactory serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.

For hosted web pairing over Tailscale HTTPS, opt in to Tailscale Serve:

```bash
npx pkfactory serve --tailscale-serve
```
...
```bash
npx pkfactory serve --tailscale-serve --tailscale-serve-port 8443
```
...
Use this when you want the desktop app to start or reuse PK Factory on another machine over SSH.
...
5. Confirm the launch. The desktop app probes the host, starts or reuses a remote PK Factory server, opens a local port forward, and saves the environment.

After setup, the renderer connects to a local forwarded HTTP/WebSocket endpoint. The remote host still owns the actual PK Factory server, projects, files, git state, terminals, and provider sessions.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.pkfactory/ssh-launch/<host-key>/`, starts or reuses a remote PK Factory server, and forwards the remote loopback port back to your desktop.

The remote host must have a compatible Node.js runtime. PK Factory uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, PK Factory first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path PK Factory uses:
...
Copy that full address into the return URL field in the same PK Factory client. Choose
...
When the PK Factory web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: PK Factory may
...
If PK Factory Connect cannot connect, check the date and time on both devices, then try again.

Finish active work before updating because the server restarts briefly. For step-by-step guidance,
see [Keeping PK Factory in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running PK Factory in the Background](./background-service.md).
...
1. `pkfactory serve` issues a one-time owner pairing token.
...
The hosted web app at `https://app.pkfactory.codes` can save a remote backend in browser local storage from a URL like:
...
Hosted pairing does not proxy traffic through PK Factory. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `pkfactory auth` to manage access after the initial pairing flow.
...
Use `pkfactory auth --help` and the nested subcommand help pages for the full reference.

### Deregister a PK Factory Connect Environment

Open your account menu and choose **PK Factory Connect** to see every environment registered to your
account. On mobile, open **Settings** → **PK Factory Connect**. Choose **Deregister** to revoke an
environment's PK Factory Connect access, remove any managed tunnel, and free its host space.
...
- Use `pkfactory auth` to revoke credentials or sessions you no longer trust.
