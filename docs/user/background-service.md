# Running PK Factory in the Background

On Linux and macOS, PK Factory can run as a background service for your user, so it is ready without
keeping a terminal open.

## Manage the Service

Install it with the latest PK Factory release:

```sh
npx pkfactory@latest service install
```
...
```sh
npx pkfactory@latest service status
```

Update or repair it:

```sh
npx pkfactory@latest service update
```

The service uses the same PK Factory version as the CLI you run. To install a nightly or an exact
version, use that version of the CLI:

```sh
npx pkfactory@nightly service update
npx pkfactory@1.2.3 service update
```

The install and update commands refuse to replace a newer service with an older version. Setup
through PK Factory Connect leaves a newer service unchanged. To downgrade, select the exact older version
and pass `--allow-downgrade`:

```sh
npx pkfactory@1.2.3 service update --allow-downgrade
```

Stop it and remove it from startup:

```sh
npx pkfactory@latest service uninstall
```

Updating restarts PK Factory briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The service runs a small stable launcher. Exact PK Factory versions are installed separately, so a
...
**Linux** uses a systemd user unit at `~/.config/systemd/user/pkfactory.service`. The service starts
...
**macOS** uses a launch agent at `~/Library/LaunchAgents/com.pkfactory.pkfactory.service.plist`. It
...
## Using It with PK Factory Connect

PK Factory Connect may offer to install the service during setup so the host stays reachable in the
background. This is only an onboarding shortcut: the service and PK Factory Connect are managed separately.

Signing out of PK Factory Connect does not remove the service. Use `pkfactory service uninstall` when you no longer
want PK Factory to start in the background.

## Troubleshooting

Run `pkfactory service status` on the server machine. An installed version alone does not mean the service
...
| `linger-unavailable`       | PK Factory could not verify the logout setting.                                  | Run `loginctl show-user "$(id -un)" --property=Linger` and check that systemd-logind is available.                                         |
| `user-manager-unavailable` | PK Factory cannot reach your systemd user manager.                               | Run `systemctl --user status` in a login session for the service user. Install your distribution's systemd user-session support if needed. |
| `service-disabled`         | The service is not enabled to start automatically.                               | Run the repair command shown by `pkfactory service status`.                                                                                |
| `service-stopped`          | The service is installed but is not running.                                     | Read the service log and `systemctl --user status pkfactory.service`, then run the displayed repair command.                               |
...
Run only the `loginctl` command with sudo. Running `pkfactory` with sudo creates a separate installation and
Connect identity for root. If an administrator is unavailable, run `pkfactory serve` in a terminal and
...
`pkfactory service status` prints the log path. The adjacent `server.trace.ndjson` file contains detailed
server traces. For failures after authorization, see [PK Factory Connect troubleshooting](./remote-access.md#pkfactory-connect-troubleshooting).
