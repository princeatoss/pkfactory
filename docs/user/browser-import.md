...
and choose a browser under **Import from**. The import copies cookies into a PK Factory browser
...
On Linux, Chromium-based browsers use your desktop keyring to protect their cookies. PK Factory
includes the keyring reader; no separate command-line tool is needed. Allow the desktop unlock
prompt if one appears. If the keyring cannot be accessed, PK Factory reports that failure when no
