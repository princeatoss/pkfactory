...
PK Factory runs Google's official Antigravity ACP agent. By default it signs in with your personal
...
not sign in this PK Factory provider.
...
2. Choose **Install Antigravity**. PK Factory downloads the official runtime from Google to that
...
6. Wait for PK Factory to confirm sign-in and load the model choices. Select an Antigravity model
...
machine as the PK Factory environment, sign-in can finish directly.
...
return URL field in PK Factory. Choose **Continue** on web or desktop, or **Complete sign-in** on
mobile. Do not change the address to your server's hostname.

Return to the same PK Factory client and environment where you started sign-in. Another client
...
a thread or bug report. PK Factory waits for Google's agent to confirm sign-in. A successful
...
are stored in plain text in PK Factory settings on that environment and are passed only to the
...
PK Factory does not download it until you choose to install it.

On web or desktop, **Update Antigravity** appears when PK Factory has a newer managed release.
...
A nonempty **Binary path** takes priority. With the field empty, PK Factory uses its managed
runtime, then an installation on the environment's `PATH`. An invalid explicit path reports
an error instead of selecting another installation. PK Factory does not update or remove manual
installations. Clear **Binary path** to use managed installation controls.

## Models and threads

The current official ACP exposes Gemini models only. PK Factory uses the model IDs and names
...
PK Factory asks you to select an available model instead of silently changing it.

Use Antigravity's native `/plan` command to request a plan. PK Factory's separate Plan mode control
is not available for this provider.

Project skills should use `.agents/skills`. PK Factory also discovers `.gemini/skills` and the legacy
...
Antigravity reads and edits workspace files through PK Factory. Each write shows up as a file
...
When the agent offers **Allow for this thread** on a shell or web tool, PK Factory shows Google's
...
PK Factory keeps thread history and file diffs. Antigravity does not support conversation rewind,
...
or reply ownership. PK Factory cannot show separate child entries or separate child replies
...
installation. PK Factory refuses removal while the runtime is in use. An explicit **Binary path**
that points into the managed runtime must be cleared first.

## Account access and errors

Google controls account eligibility, models, and usage limits. PK Factory does not report your
...
Use any retry time Google supplies. PK Factory does not switch to an API key to get past the limit.
