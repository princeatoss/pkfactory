...
The same configuration lives in `~/.pkfactory/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. PK Factory writes the built-in defaults into that file on
