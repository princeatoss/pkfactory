...
- Reads the shared production PK Factory Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `pkfactory`) with OIDC trusted publishing from the same workflow file:
...
## PK Factory Connect relay deployment
...
```sh
vp run --filter pkfactory-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `PKFACTORY_WEB_ROUTER_URL`: defaults to `https://app.pkfactory.codes`.
- `PKFACTORY_WEB_LATEST_DOMAIN`: defaults to `latest.app.pkfactory.codes`.
- `PKFACTORY_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.pkfactory.codes`.

Required Vercel domains:

- `app.pkfactory.codes`: the router domain users open, updated by stable releases.
- `latest.app.pkfactory.codes`: channel alias updated by stable releases.
- `nightly.app.pkfactory.codes`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__pkfactory/channel?channel=latest` or
`/__pkfactory/channel?channel=nightly`; the router stores the
`pkfactory_web_channel` cookie and rewrites future requests on `app.pkfactory.codes` to
...
through `/__pkfactory/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.pkfactory.codes` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `pkfactory`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `pkfactory@<version>` package available on
...
For a release smoke test, confirm `npm view pkfactory@<version> version` returns the expected version, then
...
verify that the update stops before restart and run `npx pkfactory@<version> service update` once on the
...
- Repository slug source:
  - `PKFACTORY_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
and extracts that archive into `~/.pkfactory/wsl-runtime/sha256-<archive-digest>` inside
...
script temporarily prepares the `pkfactory` package, then runs `vp pm publish --filter pkfactory ...` from the
repository root so workspace publish configuration is applied correctly.

Checklist:

1. Confirm npm org/user owns package `pkfactory` (or rename package first if needed).
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `pkfactory` with npm dist-tag
`latest`, creates a real GitHub Release, aliases the hosted app to `latest.app.pkfactory.codes` and
`app.pkfactory.codes`, and can commit a version bump to `main` in the finalize job. Do not push a test tag
...
2. Create an explicit App ID for `com.pkfactory.pkfactory` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [PK Factory Connect Clerk Setup](../internals/pkfactory-connect.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.pkfactory.pkfactory` and includes
    Associated Domains.
