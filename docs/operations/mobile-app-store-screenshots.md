...
The screenshot harness runs the real mobile application against three disposable local PK Factory
...
the software keyboard, and supplies deterministic PK Factory Connect discovery rows to the real
...
1. Creates three temporary PK Factory base directories and starts a local server for each on an available
   port.
2. Creates PK Factory, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic PK Factory review diff.
...
Every configured device defaults to dark appearance and the `pkfactory` palette, so plain
...
Pass `--theme <id>` (repeatable) or `--theme all` to capture the app's other palettes: `pkfactory`,
`pkfactory-chat`, `grove`, `ocean`, `ember`, and `iris`. The runner hands the palette to the app as a launch
...
| --------------------------------------- | ------------------------- | ----------------- | ----------------------------------------- |
| `apple/iphone-6.9/dark/pkfactory/`      | iPhone 17 Pro Max         | 1320×2868         | App Store Connect iPhone 6.9-inch         |
| `apple/iphone-6.5/dark/pkfactory/`      | disposable iPhone 14 Plus | 1284×2778         | App Store Connect iPhone 6.5-inch         |
| `apple/ipad-13/dark/pkfactory/`         | iPad Pro 13-inch (M5)     | 2752×2064         | App Store Connect iPad 13-inch, landscape |
| `google-play/phone/dark/pkfactory/`     | Pixel AVD at 420 dpi      | 1080×1920         | Google Play phone, portrait 9:16          |
| `google-play/tablet-7/dark/pkfactory/`  | Pixel AVD at 600dp width  | 1080×1920         | Google Play 7-inch tablet, portrait 9:16  |
| `google-play/tablet-10/dark/pkfactory/` | Pixel AVD at 800dp width  | 1440×2560         | Google Play 10-inch tablet, portrait 9:16 |
...
The generated tree is deliberately aligned with the store upload fields:
    ...
    ├── apple/
    │   ├── iphone-6.9/dark/pkfactory/{thread,terminal,review,threads,environments}.png
    │   ├── iphone-6.5/dark/pkfactory/{thread,terminal,review,threads,environments}.png
    │   └── ipad-13/dark/pkfactory/{thread,terminal,review,threads,environments}.png
    └── google-play/
        ├── phone/dark/pkfactory/{thread,terminal,review,threads,environments}.png
        ├── tablet-7/dark/pkfactory/{thread,terminal,review,threads,environments}.png
        └── tablet-10/dark/pkfactory/{thread,terminal,review,threads,environments}.png

A light-only run writes the same tree under `light/`; `--appearance both` writes both appearance
folders, and each requested theme adds a sibling folder next to `pkfactory/`.
...
timeout from 60 to 300 minutes). The default dispatch captures both appearances of the `pkfactory`
...
ARM64 by default for local Apple Silicon development; CI sets `PKFACTORY_SHOWCASE_ANDROID_ABI=x86_64` so the
...
`pnpm --filter @pkfactory/mobile showcase` starts Expo on its normal port, so it is not compatible with
...
The Pending rows use the production offline outbox and point at the real PK Factory and React fixture
