...
- `pkfactory-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `pkfactory-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `pkfactory-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `pkfactory-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `pkfactory-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`pkfactory-relay-traces-dev-julius` for a personal stage.
