...
PK Factory Connect uses one Clerk application for web, desktop, and mobile authentication. The relay verifies
two kinds of bearer credential: template JWTs generated from the `pkfactory-relay` template with the shared
`pkfactory-relay` audience, and Clerk OAuth tokens issued to the CLI. `verifyRelayClientBearerToken` in
...
[pkfactory-connect-auth-flow.html](./pkfactory-connect-auth-flow.html).

## Application Keys

PK Factory Connect is disabled in a fresh clone. To enable it for source builds against the production
...
```dotenv
PKFACTORY_CLERK_PUBLISHABLE_KEY=<publishable key>
PKFACTORY_CLERK_JWT_TEMPLATE=<JWT template name>
PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
PKFACTORY_RELAY_URL=https://relay.example.com
```
...
should set `PKFACTORY_CLERK_PUBLISHABLE_KEY`, `PKFACTORY_CLERK_JWT_TEMPLATE`,
`PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID`, and `PKFACTORY_RELAY_URL` before building. EAS preview and
production builds only need the Clerk publishable key, JWT template name, and relay URL in their EAS
environment.

When any client-facing public value is absent, cloud UI is omitted. The `pkfactory connect` command group is
...
deployment defaults.
`vp run --filter pkfactory-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `pkfactory connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the PK Factory CLI.
...
3. Add **both** allowed redirect URIs:
   ...
   - `https://app.pkfactory.codes/connect/callback` for the hosted out-of-band flow. This is
     `connectCallbackUrl(DEFAULT_HOSTED_APP_URL)` from `packages/shared/src/connectAuth.ts`, so a
     custom `PKFACTORY_HOSTED_APP_URL` means `$PKFACTORY_HOSTED_APP_URL/connect/callback` instead.
     Omitting it breaks headless and SSH authorization.
4. Enable the `openid`, `profile`, and `email` scopes.
5. Set `PKFACTORY_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
pkfactory connect            # default: onboarding
pkfactory connect login
pkfactory connect link       # --publish-only
pkfactory connect status     # --json
pkfactory connect publish    # --disable
pkfactory connect unlink
pkfactory connect logout
```

`pkfactory serve` is a separate top-level command, not a connect subcommand.

`pkfactory connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `pkfactory connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running PK Factory server. The next `pkfactory serve` or `pkfactory start` reconciles the relay link and launches the
managed tunnel. `pkfactory connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `pkfactory connect link` can re-enable exposure without another browser flow. `pkfactory connect
...
logout leaves it running; manage it with `pkfactory service status`, `install`, `update`, and `uninstall`.
...
| Setting | Value                          |
| ------- | ------------------------------ |
| Name    | `pkfactory-relay`              |
| Claims  | `{ "aud": "pkfactory-relay" }` |

Set `PKFACTORY_CLERK_JWT_TEMPLATE=pkfactory-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=pkfactory-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `PKFACTORY_RELAY_URL` still
...
Local desktop development uses `pkfactory-dev://app`, while packaged builds use `pkfactory://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `pkfactory-dev://app`; the production Clerk instance should only need
`pkfactory://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["pkfactory://app"]}'
```
...
The production macOS bundle ID is `com.pkfactory.pkfactory`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.pkfactory.pkfactory` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.pkfactory.pkfactory`.
...
```dotenv
PKFACTORY_APPLE_TEAM_ID=ABC1234567
PKFACTORY_MACOS_PROVISIONING_PROFILE=/absolute/path/to/pkfactory.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
PKFACTORY_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `PKFACTORY_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`PKFACTORY_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `PKFACTORY_PORT` set. Rebuild the signed app
...
PKFACTORY_PORT=13773 \
  "/Applications/PK Factory (Alpha).app/Contents/MacOS/PK Factory (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/PK Factory (Alpha).app"
codesign -d --entitlements :- "/Applications/PK Factory (Alpha).app"
```
...
Signed-in users manage PK Factory Connect under **Connections**. The settings sidebar also has dedicated
controls, rendered by `SettingsSidebarNav.tsx`: `PKFactoryConnectSidebarSignIn` in the footer shows a
**Sign in to PK Factory Connect** button while signed out, and `PKFactoryConnectSidebarAvatar` shows a Clerk
