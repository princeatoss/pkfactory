...
The launcher is the only runtime writer of `service-state.json`. `pkfactory service install` and
`pkfactory service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `pkfactory@<target>` into a unique staging directory.
...
```sh
npx pkfactory@<version> service update
```
