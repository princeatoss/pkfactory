---
name: test-pkfactory-mobile
description: Launch and test PK Factory Mobile on an iOS Simulator or Android Emulator against disposable local PK Factory environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test PK Factory Mobile

Run one focused, end-to-end mobile verification pass against disposable PK Factory state. Use the sibling [`test-pkfactory-app`](../test-pkfactory-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `PK Factory Dev`
- Bundle/package identifier: `com.pkfactorytools.pkfactory.dev`
- URL scheme: `pkfactory-dev`
...
## Start one disposable PK Factory environment

Run backend commands from the repository root. Use the ignored, worktree-local `.pkfactory` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.pkfactory` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-pkfactory-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `pkfactory-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/PKFactoryDev.xcworkspace`
- Scheme: `PKFactoryDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.pkfactorytools.pkfactory.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.pkfactorytools.pkfactory.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.pkfactorytools.pkfactory.dev
...
adb -s <emulator-serial> shell am start -W \
  ...
  com.pkfactorytools.pkfactory.dev
```
...
```bash
.agents/skills/test-pkfactory-mobile/scripts/pair-client.sh \
...
```text
pkfactory-dev://connections/new?pairingUrl=<encoded-pairing-url>&autoConnect=1
```
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in PK Factory when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from PK Factory Dev.
