...
if ! pairing_output="$({
  PKFACTORY_PORT="$server_port" node apps/server/src/bin.ts auth pairing create \
    --base-dir "$base_dir" \
...
case "$platform" in
  ...
  android)
    ...
    adb -s "$device_id" shell \
      "am start -W -a android.intent.action.VIEW -d '$deep_link' com.pkfactorytools.pkfactory.dev" \
      >/dev/null
    ;;
esac
