...
Load this reference only when inspecting or seeding local PK Factory state directly.
...
```bash
node apps/server/scripts/pkfactory-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/pkfactory-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/pkfactory-seed.sql
```
