---
name: test-pkfactory-app
description: Launch, retain, and test the PK Factory web app in isolated development environments, including first-try browser authentication with one-time pairing URLs, pairing-token recovery, worktree-safe state directories, cross-turn dev server lifecycle, and direct SQLite inspection or fixture seeding. Use when an agent needs to run PK Factory locally, iteratively test UI behavior with a human, recover from an expired or consumed pairing token, isolate dev state, or prepare test data in state.sqlite.
---

# Test PK Factory App

Use this skill for the web client. For iOS Simulator, Android Emulator, or physical-device testing against an isolated PK Factory backend, use the sibling [`test-pkfactory-mobile`](../test-pkfactory-mobile/SKILL.md) skill.

## Start an isolated web environment

1. Run commands from the repository root.
2. Choose a base directory that belongs only to the current worktree or test:
   - Use the repository's ignored `.pkfactory` directory for reusable worktree-local state.
   - Use `mktemp -d /tmp/pkfactory-test.XXXXXX` for disposable state and retain the printed absolute path.
3. Start the full web stack with `vp run dev`. Add `--share` when the user needs to open it from another tailnet device. In a linked worktree it defaults to that worktree's gitignored `.pkfactory`; pass `--home-dir <base-dir>` only when the test needs a different isolated directory.
...
Treat a base directory as disposable only when it was created or deliberately selected for the current test. Never delete or directly seed the shared `~/.pkfactory` directory. Prefer starting with a new temporary base directory over clearing state of uncertain ownership.

The worktree-local default deliberately outranks an ambient `PKFACTORY_HOME`; do not pass the shared home through to a worktree dev server.
...
Run `node apps/server/src/bin.ts pair` from the repository root. It discovers the running dev server (worktree `.pkfactory` first, same precedence as the dev runner) and prints a fresh `Pair URL` against the server's current web origin, including a `--share` tailnet origin. Pass `--base-dir <base-dir>` only when the server was started with `--home-dir`, using the identical path.
...
- Use `node apps/server/scripts/pkfactory-sqlite-state.ts query` for schema discovery and read-only checks.
- Stop the dev server before using `node apps/server/scripts/pkfactory-sqlite-state.ts exec`, then restart it with the same base directory.
...
The helper refuses to write to the shared `~/.pkfactory` directory by default and creates a database backup before each mutation.
