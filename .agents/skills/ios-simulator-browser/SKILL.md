...
description: Stream an explicit iOS Simulator through pinned serve-sim into the PK Factory in-app browser or another agent browser. Use on Apple Silicon macOS when the user should watch simulator verification live or when browser-visible simulator evidence is needed.
---
...
When running inside PK Factory, use its product-native browser MCP to open the stream. Other agent hosts may use their own browser or preview surface.
