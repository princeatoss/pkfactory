...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "pkfactory-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-pkfactory", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "pkfactory", specifier: pluginPath }],
