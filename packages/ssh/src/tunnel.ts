...
} from "@pkfactory/shared/httpReadiness";
import * as NetService from "@pkfactory/shared/Net";
import { extractJsonObject, fromLenientJson } from "@pkfactory/shared/schemaJson";
import { satisfiesSemverRange } from "@pkfactory/shared/semver";
...
export interface RemotePKFactoryRunnerOptions {
...
export interface SshEnvironmentManagerOptions {
  ...
  readonly resolveCliRunner?: Effect.Effect<RemotePKFactoryRunnerOptions>;
}
...
function sshRunnerLogFields(runner: RemotePKFactoryRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  PKFACTORY_NODE_ENGINE_RANGE=@@PKFACTORY_NODE_ENGINE_RANGE@@
  if [ -z "$PKFACTORY_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$PKFACTORY_NODE_ENGINE_RANGE" <<'NODE'
@@PKFACTORY_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for PKFACTORY_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$PKFACTORY_NODE_BIN/node" ]; then
        PATH="$PKFACTORY_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@PKFACTORY_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
PKFACTORY_NODE_SCRIPT_PATH=@@PKFACTORY_NODE_SCRIPT_PATH@@
if [ -n "$PKFACTORY_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$PKFACTORY_NODE_SCRIPT_PATH" "$@"
fi
if command -v pkfactory >/dev/null 2>&1; then
  exec pkfactory "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build (pkfactory depends on node-pty, which needs a C toolchain) leaves
# the npx cache without a pkfactory executable. \`npx --yes\` then exits 0 without
...
require_installed_pkfactory_cli() {
  PKFACTORY_CLI_PATH="$("$@" -- sh -c 'command -v pkfactory' || true)"
  if [ -n "$PKFACTORY_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no pkfactory executable, which usually means a native dependency (node-pty) failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@PKFACTORY_PACKAGE_SPEC@@ >&2
  return 1
}
if command -v npx >/dev/null 2>&1; then
  require_installed_pkfactory_cli npx --yes --package @@PKFACTORY_PACKAGE_SPEC@@ || exit 1
  exec npx --yes @@PKFACTORY_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_pkfactory_cli npm exec --yes --package @@PKFACTORY_PACKAGE_SPEC@@ || exit 1
  exec npm exec --yes @@PKFACTORY_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the pkfactory CLI and could not install @@PKFACTORY_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@PKFACTORY_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.pkfactory/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.pkfactory"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-pkfactory.sh"
RUNNER_NEXT="$STATE_DIR/run-pkfactory.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@PKFACTORY_DEFAULT_REMOTE_PORT@@" "@@PKFACTORY_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@PKFACTORY_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@PKFACTORY_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@PKFACTORY_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@PKFACTORY_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@PKFACTORY_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@PKFACTORY_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env PKFACTORY_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@PKFACTORY_READY_TIMEOUT_MS@@"; then
    printf 'Remote PK Factory server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
    if [ -s "$LOG_FILE" ]; then
...
STATE_DIR="$HOME/.pkfactory/ssh-launch/@@PKFACTORY_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.pkfactory"
RUNNER_FILE="$STATE_DIR/run-pkfactory.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemotePKFactoryRunnerScript(input?: RemotePKFactoryRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "pkfactory@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      PKFACTORY_PACKAGE_SPEC: packageSpec,
      PKFACTORY_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      PKFACTORY_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

export function buildRemoteNodeEnvScript(input?: RemotePKFactoryRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      PKFACTORY_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      PKFACTORY_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildRemoteNodeEngineCheckScript()),
    }),
  );
}

export function buildRemoteLaunchScript(input?: RemotePKFactoryRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    PKFACTORY_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    PKFACTORY_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePKFactoryRunnerScript(input)),
    PKFACTORY_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    PKFACTORY_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    PKFACTORY_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    PKFACTORY_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    PKFACTORY_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    PKFACTORY_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    PKFACTORY_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemotePKFactoryRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    PKFACTORY_STATE_KEY: remoteStateKey(target),
    PKFACTORY_RUNNER_SCRIPT: stripTrailingNewlines(buildRemotePKFactoryRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemotePKFactoryRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemotePKFactoryRunnerOptions;
...
>()("@pkfactory/ssh/tunnel/SshEnvironmentManager") {
