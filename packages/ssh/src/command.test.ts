...
import {
  ...
  resolveRemotePKFactoryCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote pkfactory package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemotePKFactoryCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "pkfactory@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemotePKFactoryCliPackageSpec({
