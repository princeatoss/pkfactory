...
import {
  ...
  buildRemotePKFactoryRunnerScript,
...
describe("ssh tunnel scripts", () => {
  it("builds the remote pkfactory runner with npx and npm fallbacks", () => {
    const script = buildRemotePKFactoryRunnerScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE });

    assert.include(script, "PKFACTORY_NODE_SCRIPT_PATH=''");
    assert.include(script, 'exec pkfactory "$@"');
    assert.include(script, "exec npx --yes 'pkfactory@latest' \"$@\"");
    assert.include(script, "exec npm exec --yes 'pkfactory@latest' -- \"$@\"");
    assert.include(script, "could not install 'pkfactory@latest'");
    assert.include(
      script,
      "require_installed_pkfactory_cli npx --yes --package 'pkfactory@latest'",
    );
    assert.include(
      script,
      "require_installed_pkfactory_cli npm exec --yes --package 'pkfactory@latest'",
    );
    assert.include(script, "npm produced no pkfactory executable");
    assert.include(script, 'prepend_path_if_dir "$HOME/.local/bin"');
    assert.include(script, `PKFACTORY_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`);
    ...
    assert.include(script, 'for PKFACTORY_NODE_BIN in "$NVM_DIR"/versions/node/*/bin');
    assert.notInclude(script, "ensure $NVM_DIR/nvm.sh is available");
  });

  it("does not hard-code a remote node engine range", () => {
    const script = buildRemotePKFactoryRunnerScript();

    assert.include(script, "PKFACTORY_NODE_ENGINE_RANGE=''");
    assert.notInclude(script, TEST_NODE_ENGINE_RANGE);
  });

  it("shell-quotes package specs in the remote pkfactory runner", () => {
    const script = buildRemotePKFactoryRunnerScript({
      packageSpec: "pkfactory@nightly; touch /tmp/pkfactory-owned",
    });

    assert.include(script, "exec npx --yes 'pkfactory@nightly; touch /tmp/pkfactory-owned' \"$@\"");
    assert.include(
      script,
      "exec npm exec --yes 'pkfactory@nightly; touch /tmp/pkfactory-owned' -- \"$@\"",
    );
    assert.include(
      script,
      "require_installed_pkfactory_cli npx --yes --package 'pkfactory@nightly; touch /tmp/pkfactory-owned'",
    );
    assert.notInclude(script, "exec npx --yes pkfactory@nightly; touch /tmp/pkfactory-owned");
  });

  it("builds the remote pkfactory runner with a node script override", () => {
    const script = buildRemotePKFactoryRunnerScript({
    ...
    assert.include(
      script,
      "PKFACTORY_NODE_SCRIPT_PATH='/Users/julius/Development/Work/codething-mvp/apps/server/dist/bin.mjs'",
    );
    assert.include(script, 'exec node "$PKFACTORY_NODE_SCRIPT_PATH" "$@"');
  });

  it("uses the remote pkfactory runner for launch and pairing scripts", () => {
    const target = {
      alias: "devbox",
    ...
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
      `PKFACTORY_NODE_ENGINE_RANGE='${TEST_NODE_ENGINE_RANGE}'`,
    );
    assert.include(
      buildRemoteLaunchScript({ nodeEngineRange: TEST_NODE_ENGINE_RANGE }),
    ...
    assert.include(buildRemoteLaunchScript(), "Remote PK Factory server did not become ready");
    ...
    assert.include(
      buildRemoteLaunchScript({ packageSpec: "pkfactory@nightly" }),
      "pkfactory@nightly",
    );
    assert.include(
    ...
    assert.include(
      buildRemotePairingScript(target, { packageSpec: "pkfactory@nightly" }),
      "pkfactory@nightly",
    );
    assert.include(
