...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@pkfactory/contracts";
...
const PUBLISHABLE_PKFACTORY_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemotePKFactoryCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_PKFACTORY_VERSION_PATTERN.test(appVersion)) {
    return `pkfactory@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "pkfactory@nightly";
  }

  return input.updateChannel === "nightly" ? "pkfactory@nightly" : "pkfactory@latest";
}
