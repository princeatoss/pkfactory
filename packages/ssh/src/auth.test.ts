  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({
        prefix: "pkfactory-ssh-askpass-test-",
      });
      ...
      assert.equal(env.PKFACTORY_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "pkfactory");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\pkfactory-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\pkfactory-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
