...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@pkfactory/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "pkfactory-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when PK Factory re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# PKFACTORY_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${PKFACTORY_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$PKFACTORY_SSH_AUTH_SECRET"
  exit 0
fi
printf 'PK Factory ssh-askpass invoked without PKFACTORY_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when PK Factory re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if PKFACTORY_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:PKFACTORY_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:PKFACTORY_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("PK Factory ssh-askpass invoked without PKFACTORY_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "pkfactory-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined
      ? {}
      : { PKFACTORY_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY || hostDisplay ? {} : { DISPLAY: "pkfactory" }),
  };
});
