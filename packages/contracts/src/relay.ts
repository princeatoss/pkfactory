...
}).annotate({ description: "Links an authenticated cloud user to a PK Factory environment." });
...
>()("@pkfactory/contracts/relay/RelayClientPrincipal") {}
...
>()("@pkfactory/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in PK Factory Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["pkfactory-mobile", "pkfactory-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "pkfactory-mobile" as const;
export const RelayWebClientId = "pkfactory-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "PK Factory Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking PK Factory environments, connecting authorized clients, and publishing agent activity.",
  );
