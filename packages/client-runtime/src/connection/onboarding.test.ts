...
function pairingHttpLayer(
  ...
  const fetchFn = ((input, init = {}) => {
    ...
    if (url.endsWith("/.well-known/pkfactory/environment")) {
  ...
  it.effect("prepares a persisted bearer registration from pairing details", () =>
    Effect.gen(function* () {
      ...
      expect(calls.map((call) => call.url)).toEqual([
        "https://remote.example.test/.well-known/pkfactory/environment",
      ...
      expect(tokenParams.get("client_label")).toBe("PK Factory Test");
    }),
  );
