...
import { EnvironmentAuthInvalidError } from "@pkfactory/contracts";
import {
...
describe("remote environment authorization", () => {
  it.effect("bootstraps bearer auth against a remote backend", () =>
    Effect.gen(function* () {
      ...
      expectFetchCall(fetch.calls, 1, {
        ...
        body: "grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Atoken-exchange&subject_token=pairing-token&subject_token_type=urn%3Apkfactory%3Aparams%3Aoauth%3Atoken-type%3Aenvironment-bootstrap&requested_token_type=urn%3Aietf%3Aparams%3Aoauth%3Atoken-type%3Aaccess_token",
      });
    }),
  );
  ...
  it.effect("exchanges managed credentials and admits websocket requests with DPoP", () =>
    Effect.gen(function* () {
      ...
      expectFetchCall(fetch.calls, 1, {
        ...
        body: "grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Atoken-exchange&subject_token=one-time-credential&subject_token_type=urn%3Apkfactory%3Aparams%3Aoauth%3Atoken-type%3Aenvironment-bootstrap&requested_token_type=urn%3Aietf%3Aparams%3Aoauth%3Atoken-type%3Aaccess_token&client_label=PK Factory+Code+Mobile&client_device_type=mobile&client_os=iOS",
      });
  ...
  it.effect("allows a client to explicitly narrow a pairing grant", () =>
    Effect.gen(function* () {
      ...
      expectFetchCall(fetch.calls, 1, {
        url: "https://remote.example.com/oauth/token",
        method: "POST",
        body: "grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Atoken-exchange&subject_token=pairing-token&subject_token_type=urn%3Apkfactory%3Aparams%3Aoauth%3Atoken-type%3Aenvironment-bootstrap&requested_token_type=urn%3Aietf%3Aparams%3Aoauth%3Atoken-type%3Aaccess_token&scope=orchestration%3Aread",
      });
    }),
  );
  ...
  it.effect("loads remote session state and websocket tickets over bearer auth", () =>
    Effect.gen(function* () {
      ...
      expectFetchCall(fetch.calls, 1, {
        url: "https://remote.example.com/.well-known/pkfactory/environment",
        method: "GET",
      });
      expectFetchCall(fetch.calls, 2, {
  ...
  it.effect("fails hung fetch requests on the configured timeout", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toBe(
        "Remote environment endpoint http://remote.example.com/.well-known/pkfactory/environment timed out after 25ms.",
      );
