...
describe("remote", () => {
  ...
  it("derives backend urls from hosted app pairing links", () => {
    expect(
      resolveRemotePairingTarget({
        pairingUrl:
          "https://app.pkfactory.codes/pair?host=https%3A%2F%2Fdesktop.tailnet.ts.net%3A44342%2F#token=pairing-token",
      }),
    ).toEqual({
      credential: "pairing-token",
  ...
  it("normalizes a protocol-relative host from a hosted pairing link", () => {
    expect(
      resolveRemotePairingTarget({
        pairingUrl:
          "https://app.pkfactory.codes/pair?host=%2F%2Fremote.example.com#token=pairing-token",
      }),
    ).toEqual({
      credential: "pairing-token",
      httpBaseUrl: "https://remote.example.com/",
  ...
  it("rejects unsupported hosted pairing backend protocols", () => {
    ...
    try {
      resolveRemotePairingTarget({
        pairingUrl:
          "https://app.pkfactory.codes/pair?host=ftp%3A%2F%2Fremote.example.com#token=pairing-token",
      });
    } catch (cause) {
      hostError = cause;
    }
