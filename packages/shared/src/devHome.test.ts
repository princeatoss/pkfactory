...
import { resolveGitWorktreePath, resolveWorktreePKFactoryHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "pkfactory-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreePKFactoryHome", () => {
  it.effect("answers with .pkfactory before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreePKFactoryHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".pkfactory"));
