...
describe("Clerk relay auth", () => {
  it("derives a custom Frontend API hostname from a Clerk publishable key", () => {
    expect(
      clerkFrontendApiHostnameFromPublishableKey(clerkPublishableKey("clerk.pkfactory.codes")),
    ).toBe("clerk.pkfactory.codes");
    expect(
      clerkFrontendApiUrlFromPublishableKey(clerkPublishableKey("clerk.pkfactory.codes")),
    ).toBe("https://clerk.pkfactory.codes");
  });
  ...
  it("reports semantic frontend API failures without inventing a cause", () => {
    ...
    const pathFrontendApi = "clerk.pkfactory.codes/path";
  ...
  it("allows standard Clerk hosts and an exact configured custom hostname", () => {
    ...
    expect(
      isAllowedClerkFrontendApiHostname("clerk.pkfactory.codes", "clerk.pkfactory.codes"),
    ).toBe(true);
    expect(isAllowedClerkFrontendApiHostname("attacker.example", "clerk.pkfactory.codes")).toBe(
      false,
    );
    expect(
      isAllowedClerkFrontendApiHostname("nested.clerk.pkfactory.codes", "clerk.pkfactory.codes"),
    ).toBe(false);
  });
});
