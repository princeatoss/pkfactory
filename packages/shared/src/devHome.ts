 ...
 * `~/.pkfactory` that a user's installed PK Factory runs against.
 *
 * A linked git worktree gets its own (gitignored) `.pkfactory`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `PKFACTORY_HOME` counts as an explicit base dir — flipping the state directory
 ...
 */
export const resolveWorktreePKFactoryHome = (
  cwd: string,
): Effect.Effect<string | undefined, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".pkfactory");
  });
